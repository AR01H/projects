...
-- ============================================================
-- 7. PAGE SECTIONS (Generic section toggling)
-- ============================================================

CREATE TABLE page_sections (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id         INT UNSIGNED NOT NULL,
    section_key     VARCHAR(150) NOT NULL,       -- 'hero','why_us','guide_through', etc.
    section_label   VARCHAR(200),
    is_visible      TINYINT(1) DEFAULT 1,        -- admin can hide/show
    sort_order      INT DEFAULT 0,
    updated_by      INT UNSIGNED,
    updated_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Controls which sections are visible on each page and their order.';
...
CREATE TABLE section_hero (
    ...
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Hero section: left text + right image. One row per page that has a hero.';

-- 9b. Highlight Scrollbar Items
...
) ENGINE=InnoDB COMMENT='Items in the scrolling highlight bar below the hero.';

-- 9c. Why You Need Us
CREATE TABLE section_why_us (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id         INT UNSIGNED NOT NULL,
    heading         VARCHAR(255),
    ...
    FOREIGN KEY (page_id)    REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Why You Need Us section header/description and more link.';

CREATE TABLE section_why_us_cards (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    ...
    sort_order      INT DEFAULT 0,
    status          ENUM('active','inactive') DEFAULT 'active',
    FOREIGN KEY (why_us_id) REFERENCES section_why_us(id) ON DELETE CASCADE,
    FOREIGN KEY (image_id)  REFERENCES media(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Individual cards under Why You Need Us.';

-- 9d. Guide Through
CREATE TABLE section_guide_through (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id         INT UNSIGNED NOT NULL,
    heading         VARCHAR(255),
    image_id        INT UNSIGNED,
    ...
    FOREIGN KEY (image_id)   REFERENCES media(id) ON DELETE SET NULL,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Guide Through section: image left, text right with bullet points.';

CREATE TABLE section_guide_through_points (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    guide_id        INT UNSIGNED NOT NULL,
    point_text      VARCHAR(500) NOT NULL,
    sort_order      INT DEFAULT 0,
    status          ENUM('active','inactive') DEFAULT 'active',
    FOREIGN KEY (guide_id) REFERENCES section_guide_through(id) ON DELETE CASCADE
) ENGINE=InnoDB COMMENT='Bullet points for Guide Through section.';

-- 9e. Stack Details (tech/tool stack badges)
CREATE TABLE section_stack_items (
    id          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id     INT UNSIGNED NOT NULL,
    heading     VARCHAR(255),
    ...
    FOREIGN KEY (page_id)  REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (icon_id)  REFERENCES media(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Tech/tool stack logos and names shown in Stack Details section.';

-- 9f. Difference From Others
CREATE TABLE section_difference (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id         INT UNSIGNED NOT NULL,
    heading         VARCHAR(255),
    information     TEXT,
    ...
    FOREIGN KEY (page_id)    REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Difference From Others section: header text and more link.';

CREATE TABLE section_difference_table (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    difference_id   INT UNSIGNED NOT NULL,
    feature_label   VARCHAR(255) NOT NULL,       -- row label
    us_value        VARCHAR(500),                -- our value
    others_value    VARCHAR(500),                -- competitor value
    sort_order      INT DEFAULT 0,
    status          ENUM('active','inactive') DEFAULT 'active',
    FOREIGN KEY (difference_id) REFERENCES section_difference(id) ON DELETE CASCADE
) ENGINE=InnoDB COMMENT='Comparison table rows for Difference From Others section.';

-- 9g. Featured Properties
CREATE TABLE section_featured_properties (
    id          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id     INT UNSIGNED NOT NULL,
    heading     VARCHAR(255),
    description TEXT,
    ...
    FOREIGN KEY (page_id)    REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Featured Properties section header.';

CREATE TABLE section_featured_properties_items (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    ...
    sort_order      INT DEFAULT 0,
    status          ENUM('active','inactive') DEFAULT 'active',
    FOREIGN KEY (section_id) REFERENCES section_featured_properties(id) ON DELETE CASCADE,
    FOREIGN KEY (image_id)   REFERENCES media(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Image+Text items within Featured Properties section.';

-- 9h. Our Experience
CREATE TABLE section_experience (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id         INT UNSIGNED NOT NULL,
    heading         VARCHAR(255),
    description     TEXT,
    ...
    FOREIGN KEY (page_id)    REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Our Experience section heading/description.';

CREATE TABLE section_experience_cards (
    id          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    section_id  INT UNSIGNED NOT NULL,
    image_id    INT UNSIGNED,
    title       VARCHAR(255) NOT NULL,
    ...
    sort_order  INT DEFAULT 0,
    status      ENUM('active','inactive') DEFAULT 'active',
    FOREIGN KEY (section_id) REFERENCES section_experience(id) ON DELETE CASCADE,
    FOREIGN KEY (image_id)   REFERENCES media(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Cards (2-3) for Our Experience section.';

-- 9i. Why Required (Video section)
CREATE TABLE section_why_required (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id         INT UNSIGNED NOT NULL,
    heading         VARCHAR(255),
    information     TEXT,
    ...
    FOREIGN KEY (page_id)    REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Why Required section header.';

CREATE TABLE section_why_required_cards (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    section_id      INT UNSIGNED NOT NULL,
    youtube_url     VARCHAR(500),                -- YouTube embed URL
    title           VARCHAR(255),
    description     TEXT,
    sort_order      INT DEFAULT 0,
    status          ENUM('active','inactive') DEFAULT 'active',
    FOREIGN KEY (section_id) REFERENCES section_why_required(id) ON DELETE CASCADE
) ENGINE=InnoDB COMMENT='YouTube video + text cards under Why Required section.';
...
CREATE TABLE services (
    ...
    FOREIGN KEY (image_id)   REFERENCES media(id) ON DELETE SET NULL,
    FOREIGN KEY (created_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Individual service records shown on Services page.';
...
) ENGINE=InnoDB COMMENT='Bullet point descriptions per service.';
...
CREATE TABLE services_page_header (
    ...
    FOREIGN KEY (page_id)    REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Services page main heading and intro text.';

-- Taxonomy pivot for services
...
) ENGINE=InnoDB COMMENT='Tags/categories attached to each service.';
...
CREATE TABLE about_page_header (
    id          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    page_id     INT UNSIGNED NOT NULL UNIQUE,
    heading     VARCHAR(255),
    information TEXT,
    ...
    FOREIGN KEY (page_id)    REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='About page main heading and intro paragraph.';
...
CREATE TABLE about_story (
    ...
    FOREIGN KEY (image_id)   REFERENCES media(id) ON DELETE SET NULL,
    FOREIGN KEY (updated_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Story section on About page: image + heading + subheading.';

CREATE TABLE about_story_points (
    id          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    story_id    INT UNSIGNED NOT NULL,
    point_text  VARCHAR(500) NOT NULL,
    sort_order  INT DEFAULT 0,
    FOREIGN KEY (story_id) REFERENCES about_story(id) ON DELETE CASCADE
) ENGINE=InnoDB COMMENT='Bullet points under the About story subheading.';
...
CREATE TABLE team_members (
    ...
    FOREIGN KEY (created_by) REFERENCES admin_users(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Team member profiles shown in About > Team Experts.';
...
CREATE TABLE about_values (
    ...
    FOREIGN KEY (page_id)  REFERENCES pages(id) ON DELETE CASCADE,
    FOREIGN KEY (image_id) REFERENCES media(id) ON DELETE SET NULL
) ENGINE=InnoDB COMMENT='Values cards (image + information) under About > Values section.';
