 ...
 * Loaded by ah-cms.php before any builder page template is included,
 * so both the plugin default template and any theme override can call these.
 *
...
function ah_render_builder_block( string $type, array $d ): void {
	...
	switch ( $type ) {
		...
		case 'faq':
			...
			?>
			<?php static $ah_faq_css_done = false; if ( ! $ah_faq_css_done ) : $ah_faq_css_done = true; ?>
<style>
.faq{border-bottom:1px solid #e5e7eb;}
.faq:first-of-type{border-top:1px solid #e5e7eb;}
.faq__q{width:100%;display:flex;justify-content:space-between;align-items:center;gap:16px;padding:20px 0;background:none;border:none;text-align:left;font-size:1rem;font-weight:600;color:var(--text-primary,#1e293b);cursor:pointer;line-height:1.5;}
.faq__q:hover{color:var(--accent,#b7791f);}
.faq__icon{flex-shrink:0;width:28px;height:28px;border-radius:50%;border:1.5px solid #e2e8f0;display:flex;align-items:center;justify-content:center;transition:transform .3s ease,border-color .3s ease;}
.faq__q[aria-expanded="true"] .faq__icon{transform:rotate(180deg);border-color:var(--accent,#b7791f);}
.faq__q[aria-expanded="true"]{color:var(--accent,#b7791f);}
.faq__a{overflow:hidden;max-height:0;transition:max-height .35s ease;}
.faq__a.is-open{max-height:600px;}
.faq__a-inner{padding:0 0 20px;font-size:.9375rem;color:var(--text-secondary,#4b5563);line-height:1.8;}
</style>
<?php endif; ?>
