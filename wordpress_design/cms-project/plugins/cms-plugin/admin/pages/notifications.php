...
<div class="wrap ah-wrap">
  ...
  <?php if ( $notice ) : list( $nt, $nm ) = explode( ':', $notice, 2 ); ?>
    <?php \Ah\Cms\Admin\Components\AdminComponents::notice( $nm, 'success' === $nt ? 'success' : 'warning' ); ?>
  <?php endif; ?>

  <div class="nl-header">
    <?php \Ah\Cms\Admin\Components\AdminComponents::pageHeader( 'bell', 'Notifications', 'Manage subscriber lists and send email notifications.' ); ?>
  ...
  <?php \Ah\Cms\Admin\Components\AdminComponents::dataTable( array(
    ...
    'actions'       => function ( $row ) use ( $filter, $page_slug ) {
      $html = '';
      if ( 'active' === $row['status'] ) {
        $html .= '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'page' => $page_slug, 'tab' => 'subscribers', 'unsub' => $row['email'], 'filter' => $filter ), admin_url( 'admin.php' ) ), 'ah_nl_unsub' ) ) . '" class="ah-btn ah-btn-secondary ah-btn-sm ah-confirm-delete" data-confirm="Mark as unsubscribed?">Unsubscribe</a> ';
      }
      $html .= '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'page' => $page_slug, 'tab' => 'subscribers', 'del_sub' => $row['id'], 'filter' => $filter ), admin_url( 'admin.php' ) ), 'ah_nl_del' ) ) . '" class="ah-btn ah-btn-danger ah-btn-sm ah-confirm-delete" data-confirm="Permanently delete this subscriber?">Delete</a>';
      return $html;
    },
  ) ); ?>
  ...
  <?php else : ?>
    <?php \Ah\Cms\Admin\Components\AdminComponents::emptyState(
      'No subscribers yet' . ( $filter ? ' with status "' . esc_html( $filter ) . '"' : '' ) . '. Use the notification signup widget on your site, or add subscribers manually above.'
    ); ?>
  <?php endif; ?>
  ...
  <?php ob_start(); ?>
    ...
    <?php elseif ( ! class_exists( 'AH_Workflow_Manager' ) ) : ?>
      <?php \Ah\Cms\Admin\Components\AdminComponents::notice( 'Rules Engine is not available. Notifications require it to send.', 'warning' ); ?>
    <?php else : ?>
    ...
    <?php endif; ?>
  <?php \Ah\Cms\Admin\Components\AdminComponents::card( 'Compose &amp; Send Notification', ob_get_clean() ); ?>

  <?php elseif ( 'history' === $active_tab ) : ?>
  <!-- ═══════════════════════ HISTORY ═══════════════════════ -->
  <?php \Ah\Cms\Admin\Components\AdminComponents::card( 'Send History', '<span style="font-size:13px;color:var(--ah-muted);">Last 50 sends</span>'
    . ( $bcast_log ? \Ah\Cms\Admin\Components\AdminComponents::dataTable( array(
      ...
      'items'         => $bcast_log,
      'empty_message' => 'No notifications sent yet.',
    ) ) : \Ah\Cms\Admin\Components\AdminComponents::emptyState( 'No notifications sent yet.' ) )
  ); ?>
  ...
  ob_start();
    ...
    <?php \Ah\Cms\Admin\Components\AdminComponents::card( '📩 Individual Mode — Available per subscriber', ob_get_clean() ); ?>
