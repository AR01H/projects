...
$rows  = AH_Enquiry_Model::get_list( 'guidance', $status_filter, $per_page, $offset );
$total = AH_Enquiry_Model::count( 'guidance', $status_filter );
$pages = $total > 0 ? (int) ceil( $total / $per_page ) : 1;
?>
<div class="wrap">
	<h1>Guidance Submissions</h1>

	<?php
	// ── Status filter tabs ───────────────────────────────────────────────────
	$all_count = AH_Enquiry_Model::count( 'guidance' );
	echo '<ul class="subsubsub">';
	$tab_url = add_query_arg( array( 'page' => 'ah-guidance', 'status_filter' => '' ), admin_url( 'admin.php' ) );
	...
	foreach ( $status_options as $key => $label ) {
		$cnt     = AH_Enquiry_Model::count( 'guidance', $key );
		$tab_url = add_query_arg( array( 'page' => 'ah-guidance', 'status_filter' => $key ), admin_url( 'admin.php' ) );
						...
						<div>
							...
							<dl style="display:grid;grid-template-columns:160px 1fr;gap:6px 12px;margin:0;font-size:0.875rem;">
								<?php foreach ( $row_data as $dk => $dv ) :
									if ( '' === (string) $dv && ! is_array( $dv ) ) continue;
	...
	<div class="tablenav bottom" style="margin-top:12px;">
		<div class="tablenav-pages">
			<?php for ( $p = 1; $p <= $pages; $p++ ) :
				$purl = add_query_arg( array( 'page' => 'ah-guidance', 'status_filter' => $status_filter, 'page_num' => $p ), admin_url( 'admin.php' ) );
				if ( $p === $page_num ) :
			?>
