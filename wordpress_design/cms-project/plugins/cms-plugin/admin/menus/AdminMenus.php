...
class AH_Admin_Menus {
	...
	public static function register(): void {
		...
		$submenus = [
			...
			['title' => 'Site Settings',        'menu' => 'Site Settings',        'slug' => 'ah-cms-settings',  'callback' => 'page_settings'     ],
	...
	private static function load( string $file ): void {
		...
		$plugin_alt  = AH_PLUGIN_DIR . '/admin/' . $file . '.php';
		$path        = file_exists( $theme_path ) ? $theme_path : ( file_exists( $plugin_path ) ? $plugin_path : $plugin_alt );
		if ( file_exists( $path ) ) {
			include $path;
		} else {
	...
	public static function page_featured_in()    { self::load( 'FeaturedIn'    ); }
	public static function page_spotlights()     { self::load( 'spotlights'     ); }
	public static function page_settings()       { self::load( 'settings'       ); }
	public static function page_global_settings(){ self::load( 'GlobalSettings'); }
	...
	public static function page_news_bar()       { self::load( 'NewsBar'       ); }
	...
	public static function page_client_stories() { self::load( 'ClientStories' ); }
	public static function page_taxonomy()       { self::load( 'taxonomy'       ); }
	public static function page_audit()          { self::load( 'AuditLog'      ); }
	...
	public static function page_static_pages()   { self::load( 'StaticPages'   ); }
	public static function page_redirects()      { self::load( 'RedirectRules'  ); }
	public static function page_custom_code()    { self::load( 'CustomCode'     ); }
	public static function page_visitors()        { self::load( 'visitors'       ); }
	public static function page_analytics()      { self::load( 'analytics'      ); }
	public static function page_ref_notes()      { self::load( 'ReferenceNotes' ); }
