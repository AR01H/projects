...
<div class="wrap ah-wrap">
	<?php \Ah\Cms\Admin\Components\AdminComponents::pageHeader( 'randomize', 'Workflow Manager', 'Automate anything. Define triggers, conditions, and actions to send emails, WhatsApp messages, or call any API.' ); ?>
	...
	\Ah\Cms\Admin\Components\AdminComponents::tabBarUrl( $wf_tabs, $view, 'ah-workflow-manager', 'view' );
	?>
...
<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
	...
	<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'page' => 'ah-workflow-manager', 'delete' => $editing->id ), admin_url( 'admin.php' ) ), 'ah_del_rule' ) ); ?>"
	   class="ah-btn ah-btn-danger ah-confirm-delete"
	   data-title="Delete Rule" data-confirm="This workflow rule will be permanently removed.">Delete Rule</a>
	<?php endif; ?>
</div>
</form>
...
<div class="re-section" style="border-color:#fbbf24;background:#fffbeb">
	...
	<form method="post">
		...
		<button type="submit" class="ah-btn ah-btn-primary ah-confirm-delete"
		        <?php echo 0 === $_pend ? 'disabled style="opacity:.5;cursor:not-allowed"' : ''; ?>
		        data-title="Process Pending" data-confirm="Run all <?php echo esc_attr( number_format( $_pend ) ); ?> pending actions now?">
...
<div style="display:flex;justify-content:space-between;align-items:center;margin:0 0 10px">
	...
	<?php if ( $total_logs > 0 ) : ?>
	<form method="post" style="margin:0;" class="ah-confirm-delete" data-title="Clear All Logs" data-confirm="Delete ALL trigger log entries? This cannot be undone.">
			...
			<tr>
				...
				<td style="white-space:nowrap">
					...
					<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'page' => 'ah-workflow-manager', 'view' => 'logs', 'del_log' => $lg->id ), admin_url( 'admin.php' ) ), 'ah_del_log' ) ); ?>"
					   class="ah-btn ah-btn-danger ah-btn-sm ah-confirm-delete" data-title="Delete Log" data-confirm="This log entry will be removed.">">×</a>
				</td>
			</tr>
			...
			<tr>
				...
				<td style="white-space:nowrap">
					...
					<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'page' => 'ah-workflow-manager', 'view' => 'evallog', 'del_evallog' => $ev->id ), admin_url( 'admin.php' ) ), 'ah_del_evallog' ) ); ?>"
					   class="ah-btn ah-btn-danger ah-btn-sm ah-confirm-delete" data-title="Delete Entry" data-confirm="Delete this entry?">×</a>
				</td>
			</tr>
			<?php if ( $ev_ctx ) : ?>
			...
			<tr>
				...
				<td>
					<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'page' => 'ah-workflow-manager', 'view' => 'blocked', 'unblock' => rawurlencode( $be ) ), admin_url( 'admin.php' ) ), 'ah_unblock' ) ); ?>"
					   class="ah-btn ah-btn-secondary ah-btn-sm ah-confirm-delete"
					   data-title="Unblock Email" data-confirm="Unblock this email?">Unblock</a>
				</td>
			</tr>
			<?php endforeach; ?>
		</tbody>
