...
class AH_Ajax_Handlers {
	...
	// -------------------------------------------------------------------------
	// ah_contact_submit - public: contact enquiry form submission
	// -------------------------------------------------------------------------
	public static function handle_contact_submit(): void {
		...
		if ( '' === $name ) {
			wp_send_json_error( array( 'message' => 'Your name is required.' ) );
		}
		if ( ! is_email( $email ) ) {
			wp_send_json_error( array( 'message' => 'Please enter a valid email address.' ) );
		}
		if ( '' === $message ) {
			wp_send_json_error( array( 'message' => 'Please tell us how we can help.' ) );
		}
		if ( ! $consent ) {
			wp_send_json_error( array( 'message' => 'Please agree to the privacy policy.' ) );
		}
	...
	// -------------------------------------------------------------------------
	// ah_guidance_submit - public: guidance request form submission
	// -------------------------------------------------------------------------
	public static function handle_guidance_submit(): void {
		...
		if ( '' === $help_with ) {
			wp_send_json_error( array( 'message' => 'Please select what you need help with.' ) );
		}
		if ( '' === $name ) {
			wp_send_json_error( array( 'message' => 'Your name is required.' ) );
		}
		...
		if ( '' === $requirement ) {
			wp_send_json_error( array( 'message' => 'Please describe your requirement.' ) );
		}
	...
	// -------------------------------------------------------------------------
	// ah_update_enquiry - admin: update status + notes for an enquiry
	// -------------------------------------------------------------------------
