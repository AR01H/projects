...
class AH_Taxonomy_Model extends AH_Model_Base {
	...
	public function get_paginated( int $page = 1, string $search = '', ?int $type_id = null, int $parent_id = 0, string $status = '' ): array {
		$where    = array();
		$where_in = array();
		if ( $search ) {
		...
		if ( $parent_id ) { $where[] = 'parent_term_id = %d'; $where_in[] = $parent_id; }
		if ( $status && in_array( $status, array( 'active', 'inactive' ), true ) ) {
			$where[] = 'status = %s';
			$where_in[] = $status;
		}
		$args = array( 'order_by' => 'id', 'order' => 'DESC' );
	...
	public function get_children( int $parent_id ): array {
		return $this->all( array(
			'where'    => "parent_term_id = {$parent_id}",
			'order_by' => 'sort_order',
			'order'    => 'ASC',
		) );
	}
	...
	public function get_parent_terms( ?int $type_id = null ): array {
		$where = '(parent_term_id IS NULL OR parent_term_id = 0)';
	...
	public function count_children( int $parent_id ): int {
		...
		return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM `{$table}` WHERE parent_term_id = %d", $parent_id ) );
	}
}
