...
$table_exists = $wpdb->get_var( $wpdb->prepare(
	'SHOW TABLES LIKE %s', $wpdb->prefix . 'ch_franchise_enquiries'
) ) === $wpdb->prefix . 'ch_franchise_enquiries';

$statuses = class_exists( 'CH_Franchise_Data' ) ? CH_Franchise_Data::statuses() : [];

// ── Route: detail view ────────────────────────────────────────────────────────
$view       = sanitize_key( $_GET['view'] ?? 'list' );
$enquiry_id = absint( $_GET['enquiry_id'] ?? 0 );

if ( $view === 'detail' && $enquiry_id && $table_exists ) {
	$enquiry = class_exists( 'CH_Franchise_Data' ) ? CH_Franchise_Data::get_by_id( $enquiry_id ) : null;
	if ( ! $enquiry ) {
		echo '<div class="wrap ch-admin-wrap"><div class="ch-notice ch-notice--warning">Enquiry #' . esc_html( $enquiry_id ) . ' not found.</div></div>';
		return;
	}

	// ── Handle POST ───────────────────────────────────────────────────────────
	$saved_msg = '';
	if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['ch_franchise_nonce'] ) ) {
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ch_franchise_nonce'] ) ), 'ch_franchise_action_' . $enquiry_id ) ) {
	...
	$logs        = class_exists( 'CH_Franchise_Data' ) ? CH_Franchise_Data::get_logs( $enquiry_id ) : [];
	$status_info = $statuses[ $enquiry->status ] ?? [ 'label' => $enquiry->status, 'color' => '#666' ];
	$list_url    = admin_url( 'admin.php?page=ch-franchise-enquiries' );
	?>
	<div class="wrap ch-admin-wrap">
		<h1>
			🌿 Franchise Enquiry #<?php echo esc_html( $enquiry_id ); ?>
		...
		<div style="display:grid;grid-template-columns:1fr 340px;gap:1.5rem;align-items:start;">
			...
			<div>
				...
				<div class="ch-card">
					...
					<?php if ( $enquiry->admin_notes ) : ?>
						<pre style="white-space:pre-wrap;font-family:inherit;font-size:.85rem;background:#f9f9f9;padding:.8rem;border-radius:4px;border:1px solid #e0e0e0;"><?php echo esc_html( $enquiry->admin_notes ); ?></pre>
					<?php else : ?>
						<p style="color:#999;">No notes yet.</p>
					<?php endif; ?>
					<form method="post" style="margin-top:1rem;">
						<?php wp_nonce_field( 'ch_franchise_action_' . $enquiry_id, 'ch_franchise_nonce' ); ?>
						<input type="hidden" name="ch_franchise_action" value="add_note">
			...
			<div>
				<div class="ch-card" style="position:sticky;top:32px;">
					...
					<form method="post" style="margin-top:1rem;">
						<?php wp_nonce_field( 'ch_franchise_action_' . $enquiry_id, 'ch_franchise_nonce' ); ?>
						<input type="hidden" name="ch_franchise_action" value="update_status">
						...
						<select name="new_status" style="width:100%;padding:.5rem;border:1px solid #ddd;border-radius:4px;margin-bottom:.8rem;">
							<?php foreach ( $statuses as $slug => $info ) : ?>
								<option value="<?php echo esc_attr( $slug ); ?>" <?php selected( $enquiry->status, $slug ); ?>>
...
$stats       = ( $table_exists && class_exists( 'CH_Franchise_Data' ) ) ? CH_Franchise_Data::count_by_status() : [];
$total_all   = array_sum( $stats );
$total       = 0;
$enquiries   = [];

if ( $table_exists && class_exists( 'CH_Franchise_Data' ) ) {
	$query_args = [ 'status' => $filter_status, 'search' => $search, 'limit' => $per_page, 'offset' => $offset ];
	$enquiries  = CH_Franchise_Data::get_all( $query_args );
	$total      = CH_Franchise_Data::count( $query_args );
}

$total_pages = $total > 0 ? (int) ceil( $total / $per_page ) : 1;

function ch_franchise_list_url( array $extra = [] ): string {
	return add_query_arg( array_merge( [ 'page' => 'ch-franchise-enquiries' ], $extra ), admin_url( 'admin.php' ) );
}
?>
<div class="wrap ch-admin-wrap">
	...
	<?php if ( ! $table_exists ) : ?>
		<div class="ch-notice ch-notice--warning">
			⚠️ The franchise enquiries table doesn't exist yet.
	...
	<div class="ch-card" style="padding:1rem 1.5rem;">
		<div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
			<div style="display:flex;gap:.4rem;flex-wrap:wrap;">
				<a href="<?php echo esc_url( ch_franchise_list_url() ); ?>"
					class="button<?php echo ! $filter_status ? ' button-primary' : ''; ?>">
					All (<?php echo esc_html( $total_all ); ?>)
				</a>
				<?php foreach ( $statuses as $slug => $info ) : ?>
					<a href="<?php echo esc_url( ch_franchise_list_url( [ 'status' => $slug ] ) ); ?>"
			...
			<form method="get" style="margin-left:auto;">
				<input type="hidden" name="page" value="ch-franchise-enquiries">
				<?php if ( $filter_status ) : ?>
					<input type="hidden" name="status" value="<?php echo esc_attr( $filter_status ); ?>">
				<?php endif; ?>
				<input type="search" name="s" value="<?php echo esc_attr( $search ); ?>"
					placeholder="Search name, email, city…" style="width:240px;padding:.4rem .6rem;border:1px solid #ddd;border-radius:4px;">
				<button type="submit" class="button">Search</button>
				<?php if ( $search ) : ?>
					<a href="<?php echo esc_url( ch_franchise_list_url( $filter_status ? [ 'status' => $filter_status ] : [] ) ); ?>" class="button">Clear</a>
				<?php endif; ?>
			</form>
		</div>
	</div>
	...
	<div class="ch-card" style="padding:0;overflow:hidden;">
		<?php if ( empty( $enquiries ) ) : ?>
			<p style="padding:1.5rem;color:#666;">
				<?php echo $search ? 'No enquiries matching your search.' : 'No franchise enquiries yet.'; ?>
			</p>
		<?php else : ?>
			<table class="widefat striped" style="border:0;">
				...
				<tbody>
					<?php foreach ( $enquiries as $row ) :
						...
						$detail_url = ch_franchise_list_url( [ 'view' => 'detail', 'enquiry_id' => $row->id ] );
					?>
						<tr>
			...
			<?php if ( $total_pages > 1 ) : ?>
				<div style="padding:1rem 1.5rem;display:flex;gap:.5rem;align-items:center;">
					<span style="color:#666;font-size:.85rem;">
						...
						(<?php echo esc_html( $total ); ?> enquiries)
					</span>
					<?php if ( $current_page > 1 ) : ?>
						<a href="<?php echo esc_url( ch_franchise_list_url( array_filter( [ 'status' => $filter_status, 's' => $search, 'paged' => $current_page - 1 ] ) ) ); ?>" class="button">← Prev</a>
					<?php endif; ?>
					<?php if ( $current_page < $total_pages ) : ?>
						<a href="<?php echo esc_url( ch_franchise_list_url( array_filter( [ 'status' => $filter_status, 's' => $search, 'paged' => $current_page + 1 ] ) ) ); ?>" class="button">Next →</a>
					<?php endif; ?>
				</div>
			<?php endif; ?>
		<?php endif; ?>
