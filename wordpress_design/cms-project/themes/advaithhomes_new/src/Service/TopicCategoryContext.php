...
class TopicCategoryContext {
	...
	public static function getContext() {
		...
		if ( ! empty( $term->parent_term_id ) ) {
			$parent = $repo->get_parent_term_by_id( (int) $term->parent_term_id );
		}
		...
		if ( $parent && (int) $term->id ) {
			if ( ! empty( $term->parent_term_id ) ) {
				$sibs = $repo->get_sibling_terms( (int) $term->parent_term_id, (int) $term->id, 'parent_term_id', 6 );
			} else {
				$sibs = $repo->get_sibling_terms( (int) $parent->id, (int) $term->id, 'parent_id', 6 );
			}
		...
		if ( $term ) {
			$_post_ids = $repo->get_term_post_ids( (int) $term->id );
		...
		if ( $parent ) {
			if ( ! empty( $term->parent_term_id ) ) {
				$all_sibs = $repo->get_sidebar_sibling_terms( (int) $term->parent_term_id, (int) $term->id, 'parent_term_id' );
			} else {
				$all_sibs = $repo->get_sidebar_sibling_terms( (int) $parent->id, (int) $term->id, 'parent_id' );
			}
