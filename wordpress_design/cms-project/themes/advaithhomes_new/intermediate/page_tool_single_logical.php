...
function adn_calculator_single_get_context( $key ) {
	...
	// ── Guide link ────────────────────────────────────────────────────────
	$guide = array(
		'label' => ( isset( $meta['guide_label'] ) && '' !== $meta['guide_label'] ) ? (string) $meta['guide_label'] : adn_term( 'calculators_page.guide_link_label', 'Read the full guide →' ),
	...
	$_eh_title  = ( isset( $pg['sidebar_help_title'] )     && '' !== $pg['sidebar_help_title'] )     ? (string) $pg['sidebar_help_title']     : adn_term( 'calculators_page.sidebar_help_title', 'Need Expert Help?' );
	$_eh_text   = ( isset( $pg['sidebar_help_text'] )      && '' !== $pg['sidebar_help_text'] )      ? (string) $pg['sidebar_help_text']      : adn_term( 'calculators_page.sidebar_help_text', 'Speak to one of our industry experts today.' );
	...
	$newsletter_sidebar = array(
		'heading'      => ! empty( $pg['sidebar_nl_heading'] )     ? (string) $pg['sidebar_nl_heading']     : adn_term( 'calculators_page.sidebar_nl_heading', 'Stay in the Know' ),
		'description'  => ! empty( $pg['sidebar_nl_desc'] )        ? (string) $pg['sidebar_nl_desc']        : adn_term( 'calculators_page.sidebar_nl_desc', 'Get news, guides and calculator tips straight to your inbox.' ),
		'placeholder'  => ! empty( $pg['sidebar_nl_placeholder'] ) ? (string) $pg['sidebar_nl_placeholder'] : adn_term( 'calculators_page.sidebar_nl_placeholder', 'Your email address' ),
		'button_label' => ! empty( $pg['sidebar_nl_btn_label'] )   ? (string) $pg['sidebar_nl_btn_label']   : adn_term( 'calculators_page.sidebar_nl_btn', 'Subscribe' ),
	...
	return array(
		...
		'sidebar'       => array(
			...
			'news_mini'   => ! empty( $news_mini_items ) ? array(
				...
				'view_all' => array( 'label' => adn_term( 'content.view_all_news', 'View all →' ), 'url' => SITE_NEWS_URL ),
			) : array(),
		),
	);
}
