 ...
 * It measured an element called `.app-mh__title-shell` - the HTML span the
...
(function () {
  ...
  function fit(title) {
    var svg = title.querySelector(".app-mh__title-svg");
    if (!svg) {
      return;
    }

    var group = svg.querySelector(".app-mh__type");
    if (!group) {
      return;
    }
    ...
    var face = group.querySelector(".app-mh__type-face");
    if (!face) {
      return;
    }

    var base =
      parseFloat(group.getAttribute("data-nt-mh-base")) ||
