...
<section id="<?php echo esc_attr( $section_id ); ?>" class="<?php echo esc_attr( $section_class ); ?>">
	<div class="<?php echo esc_attr( $container_class ); ?>">
		...
		<div class="<?php echo esc_attr( $layout_class ); ?>">
			...
			<!-- ── Visual panel ──────────────────────────────────────────────── -->
			<?php 
			$has_dynamic_images = ! $visual_image && count(array_filter($items, fn($c) => !empty($c['image']))) > 0;
			if ( $visual_image || $has_dynamic_images ) : ?>
						...
						<?php foreach ( $items as $i => $cert ) : ?>
							<?php if ( !empty($cert['image']) ) : ?>
								<button type="button" 
								        ...
								        data-img="<?php echo esc_url($cert['image']); ?>"
								        aria-label="View <?php echo esc_attr($cert['title']); ?> certificate fullscreen">
									<img src="<?php echo esc_url( $cert['image'] ); ?>"
...
<?php if ( $has_dynamic_images ) : 
	$dialog_content = '<img id="' . esc_attr($section_id) . '-popup-img" src="" alt="Certificate" style="max-width:100%;height:auto;display:block;margin:0 auto;border-radius:12px;">';
	get_template_part( 'components/dialogs/generic-dialog', null, [
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function setActive(index) {
		...
		visuals.forEach(function(v) { 
			var isMatch = parseInt(v.dataset.carouselIndex, 10) === index;
	...
	var prevBtn = section.querySelector('#<?php echo esc_js($prev_id); ?>');
	var nextBtn = section.querySelector('#<?php echo esc_js($next_id); ?>');
	
	function getActiveIndex() {
		var active = Array.prototype.findIndex.call(cards, function(c) { return c.classList.contains('active'); });
		return active > -1 ? active : 0;
	}
	
	if (prevBtn) prevBtn.addEventListener('click', function() { setActive(getActiveIndex() - 1); });
	if (nextBtn) nextBtn.addEventListener('click', function() { setActive(getActiveIndex() + 1); });
});
</script>
<?php endif; ?>
