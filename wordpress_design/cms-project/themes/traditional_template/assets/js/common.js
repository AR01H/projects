 ...
 * fields through NT.ajax(), shows the result in its .app-form-status element,
...
(function () {
  ...
  function ajax(action, data) {
    ...
    body.append("action", "app_" + action);
  ...
  function initAjaxForms() {
    ...
    forms.forEach(function (form) {
      if (form.dataset.ntAjaxBound) {
        return;
      }
      form.dataset.ntAjaxBound = "1";

      var action = form.getAttribute("data-nt-ajax-form");
      var status = form.querySelector(".app-form-status");
        ...
        ajax(action, data)
          .then(function (res) {
            ...
            var msg =
              ...
              (ok ? "Thank you!" : "Something went wrong. Please try again.");
            setStatus(msg, !ok);
            if (ok) {
              form.reset();
            }
   ...
   *   - [data-nt-close]           closes its nearest .app-modal
   *   - clicking the .app-modal backdrop, or Escape, also closes it
   */
  ...
  function initModals() {
    document.querySelectorAll("[data-nt-open]").forEach(function (trigger) {
      if (trigger.dataset.ntOpenBound) {
        return;
      }
      ...
      trigger.addEventListener("click", function (e) {
        e.preventDefault();
        openModal(
          document.getElementById(trigger.getAttribute("data-nt-open")),
        );
      });
    });
    document.querySelectorAll(".app-modal").forEach(function (modal) {
      if (modal.dataset.ntModalBound) {
        return;
      }
      ...
      modal.addEventListener("click", function (e) {
        if (
          e.target === modal ||
          e.target.hasAttribute("data-nt-close") ||
          e.target.closest("[data-nt-close]")
        ) {
          closeModal(modal);
        }
      });
   ...
   * components/parts/generic-multistep-form.php (.app-bk-step / .app-bk-next /
   * .app-bk-back / .app-bk-prog-step / .app-bk-prog-fill) inside a modal.
  ...
  function initWizards() {
    document.querySelectorAll("form[data-nt-wizard]").forEach(function (form) {
      if (form.dataset.ntWizBound) {
        return;
      }
      form.dataset.ntWizBound = "1";

      var root = form.closest(".app-bk-modal-scroll") || form.parentNode;
      var steps = Array.prototype.slice.call(
        form.querySelectorAll(".app-bk-step"),
      );
      var progSteps = Array.prototype.slice.call(
        root.querySelectorAll(".app-bk-prog-step"),
      );
      var progFill = root.querySelector(".app-bk-prog-fill");
      var msg = form.querySelector(".app-form-feedback");
      ...
      function show(n) {
        ...
        if (progFill && total > 1) {
          progFill.style.width = ((current - 1) / (total - 1)) * 100 + "%";
        }
        say("");
      }
      function stepValid() {
        ...
        stepEl.querySelectorAll("[required]").forEach(function (f) {
          ...
          f.classList.toggle("app-invalid", empty);
          if (empty) {
            ok = false;
          }
        });
        if (!ok) {
          say("Please fill in the required fields to continue.", "error");
        }
        return ok;
      }

      form.querySelectorAll(".app-bk-next").forEach(function (b) {
        b.addEventListener("click", function () {
          if (stepValid()) {
            show(parseInt(b.dataset.next, 10));
          }
        });
      });
      form.querySelectorAll(".app-bk-back").forEach(function (b) {
        b.addEventListener("click", function () {
          show(parseInt(b.dataset.back, 10));
        });
      });
      progSteps.forEach(function (p) {
      ...
      form.addEventListener("submit", function (e) {
        ...
        ajax(action, data)
          .then(function (res) {
            if (res && res.success) {
              var prog = root.querySelector(".app-bk-progress");
              if (prog) {
              ...
              if (last) {
                last.innerHTML =
                  '<div class="app-wiz-success">' +
                  '<div class="app-wiz-success__icon">🎉</div>' +
                  "<h3>All done!</h3>" +
                  "<p>" +
                  escapeHtml(
   ...
   * Markup: components/media-carousel.php (.app-media-carousel).
   */
  function initMediaCarousels() {
    document.querySelectorAll(".app-media-carousel").forEach(function (root) {
      if (root.dataset.ntMediaBound) {
        return;
      }
      root.dataset.ntMediaBound = "1";

      var slides = Array.prototype.slice.call(
        root.querySelectorAll(".app-media-slide"),
      );
      var dots = Array.prototype.slice.call(
        root.querySelectorAll("[data-nt-media-dot]"),
      );
      if (slides.length < 2) {
        var only = slides[0] && slides[0].querySelector("video");
        if (only) {
          only.play && only.play().catch(function () {});
        }
        return;
      }
      ...
      dots.forEach(function (d) {
        d.addEventListener("click", function () {
          go(parseInt(d.getAttribute("data-nt-media-dot"), 10));
          restart();
        });
      });
   ...
   * the blended transparent-header state (body.app-hero-top in vintage.css).
  ...
  function initLightbox() {
    ...
    function build() {
      ...
      overlay.innerHTML =
        '<button type="button" class="app-lightbox__close" aria-label="Close">&times;</button>' +
        '<button type="button" class="app-lightbox__nav app-lightbox__nav--prev" aria-label="Previous image">&#8249;</button>' +
        '<figure class="app-lightbox__figure">' +
        '<img class="app-lightbox__img" src="" alt="">' +
        '<figcaption class="app-lightbox__cap"></figcaption>' +
        "</figure>" +
        '<button type="button" class="app-lightbox__nav app-lightbox__nav--next" aria-label="Next image">&#8250;</button>';
      document.body.appendChild(overlay);
      imgEl = overlay.querySelector(".app-lightbox__img");
      capEl = overlay.querySelector(".app-lightbox__cap");

      overlay
        .querySelector(".app-lightbox__close")
        .addEventListener("click", close);
      overlay
        .querySelector(".app-lightbox__nav--prev")
        .addEventListener("click", function (e) {
          e.stopPropagation();
          step(-1);
        });
      overlay
        .querySelector(".app-lightbox__nav--next")
        .addEventListener("click", function (e) {
          e.stopPropagation();
          step(1);
        });
    ...
    function show() {
      ...
      overlay.querySelector(".app-lightbox__nav--prev").hidden = !multi;
      overlay.querySelector(".app-lightbox__nav--next").hidden = !multi;
    }
    function step(dir) {
      if (!current.length) {
        return;
    ...
    function open(list, i, from) {
      ...
      document.body.classList.add("app-lightbox-open");
      overlay.querySelector(".app-lightbox__close").focus();
    }
    function close() {
      if (!overlay) {
        return;
    ...
    groups.forEach(function (group) {
      ...
      imgs.forEach(function (img, i) {
        img.classList.add("app-lightbox-trigger");
  ...
  function initScrollUI() {
    ...
    /* THE PAGE HAD TWO BACK-TO-TOP BUTTONS.
		   The footer already ships a vintage one (#app-scroll-to-top, the
    ...
    var toTop = document.getElementById("app-scroll-to-top");
    var ownsButton = false;

    if (!toTop) {
      toTop = document.createElement("button");
      toTop.type = "button";
      toTop.className = "app-totop";
    ...
    toTop.addEventListener("click", function () {
      ...
      var target = document.querySelector("header a, header button, #app-main");
      if (target) {
        if (!target.hasAttribute("tabindex") && target.id === "app-main") {
   ...
   *
   * Skipped: anything marked .app-no-rough (logos, mascots, ornaments) and
   * every SVG, which is line art rather than photography.
   */
  function initRoughPhotos() {
    var imgs = document.querySelectorAll(".site-main img:not(.app-no-rough)");
    var n = 0;
      ...
      function step() {
        ...
        if (card && card.offsetWidth) {
          var perPage = Math.max(
            1,
            Math.floor(track.clientWidth / (card.offsetWidth + gap)),
          );
  ...
  function initVideoFacade() {
    ...
    Array.prototype.forEach.call(triggers, function (trigger) {
      trigger.addEventListener("click", function (e) {
        ...
        frame.className = "app-videofeat__frame";
