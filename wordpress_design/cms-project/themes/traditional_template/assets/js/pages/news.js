...
(function () {
  ...
  function card(item) {
    var article = NT.el("article", "app-card");
    var link = NT.el("a", "app-card-link");
    link.href = item.url;

    var media = NT.el("div", "app-card-media");
    if (item.thumb) {
      ...
      media.appendChild(NT.el("span", "app-card-media-empty"));
    }

    var body = NT.el("div", "app-card-body");
    body.appendChild(NT.el("h3", "app-card-title", item.title));
    body.appendChild(NT.el("p", "app-card-excerpt", item.excerpt));
    body.appendChild(NT.el("span", "app-card-meta", item.date));
  ...
  function load(replace) {
    ...
    NT.rest("posts", {
      ...
      .catch(function () {
        say("Could not load news. Please try again.");
      })
      .finally(function () {
        if (moreBtn) {
