...
/* ==========================================================================
   ...
   Initialises every .app-mvs on the page.
...
(function () {
  ...
  function initMiniVideo(root) {
    ...
    var track = root.querySelector(".app-mvs-track");
    if (!track) return;
    var prev = root.querySelector(".app-mvs-arrow--prev");
    var next = root.querySelector(".app-mvs-arrow--next");

    /* ── Scrolling ──────────────────────────────────────────────────── */
    function pageAmount() {
      return Math.max(200, Math.round(track.clientWidth * 0.85));
    }
    ...
    if (prev)
      prev.addEventListener("click", function () {
        track.scrollBy({ left: -pageAmount(), behavior: "smooth" });
      });
    if (next)
    ...
    function stopThumb(thumb) {
      var frame = thumb.querySelector(".app-mvs-frame");
      if (frame) {
    ...
    function stopOthers(except) {
      track
        .querySelectorAll(".app-mvs-play-btn.is-playing")
        .forEach(function (t) {
    ...
    function playInto(thumb) {
      ...
      frame.className = "app-mvs-frame";
      thumb.appendChild(frame);
      thumb.classList.add("is-playing");
    }

    track.querySelectorAll(".app-mvs-play-btn").forEach(function (thumb) {
  ...
  function boot() {
    document.querySelectorAll(".app-mvs").forEach(initMiniVideo);
  }
...
/* ==========================================================================
   ...
   Initialises every .app-vs on the page; reads config from data- attributes.
...
(function () {
  ...
  function initVideoScroll(root) {
    ...
    var track = root.querySelector(".app-vs-track");
    var vp = root.querySelector(".app-vs-viewport");
    if (!track || !vp) return;

    var slides = Array.prototype.slice.call(
      track.querySelectorAll(".app-vs-slide"),
    );
    var dots = Array.prototype.slice.call(root.querySelectorAll(".app-vs-dot"));
    ...
    function stopMedia() {
      ...
      track
        .querySelectorAll('.app-vs-media[data-yt-open="1"]')
        .forEach(function (media) {
          var btn = media.querySelector(".app-vs-yt");
    ...
    /* Arrows */
    var prev = root.querySelector(".app-vs-arrow--prev");
    var next = root.querySelector(".app-vs-arrow--next");
    if (prev)
      prev.addEventListener("click", function () {
        go(index - 1);
        restart();
    ...
    function playFacade(btn) {
      var media = btn.closest(".app-vs-media");
    ...
    track.querySelectorAll(".app-vs-yt").forEach(function (btn) {
  ...
  function boot() {
    document.querySelectorAll(".app-vs").forEach(initVideoScroll);
  }
...
(function (window) {
  ...
  CHCarousel.prototype._build = function () {
    var el = this.el;
    this.track = el.querySelector(".app-carousel__track");
    this.items = Array.from(el.querySelectorAll(".app-carousel__item"));
    this.viewport = el.querySelector(".app-carousel__viewport");
    this.navEl = el.querySelector(".app-carousel__nav");
    this.dots = Array.from(el.querySelectorAll(".app-carousel__dot"));
  ...
  CHCarousel.prototype._visibleCount = function () {
    ...
    var raw = getComputedStyle(this.track)
      .getPropertyValue("--cc-items-visible")
      .trim();
  ...
  CHCarousel.prototype.next = function () {
    this.goTo(this._index + 1);
  };
  CHCarousel.prototype.prev = function () {
    this.goTo(this._index - 1);
  };
  ...
  CHCarousel.prototype._initSelector = function () {
    ...
    this.items.forEach(function (item, i) {
      var card = item.querySelector(".app-card--selector");
      if (!card) return;
      card.addEventListener("click", function () {
        self.el.querySelectorAll(".app-card--selector").forEach(function (c) {
    ...
    var first = this.el.querySelector(".app-card--selector");
  ...
  CHCarousel.init = function (scope) {
    (scope || document)
      .querySelectorAll(".app-carousel:not([data-carousel-init])")
      .forEach(function (el) {
...
/* ==========================================================================
   ...
   Required DOM (all derived from prefix, e.g. 'bk'):
     #app-{prefix}-form     the <form>            (or pass opts.formId)
     #app-{prefix}-modal    the modal wrapper
     #app-{prefix}-open     the button that opens it
     #app-{prefix}-msg      feedback element
     #app-{prefix}-submit   submit button
     #app-{prefix}-summary  summary container (filled on the last step)
     [data-{prefix}-close] any close trigger
     .app-bk-step / .app-bk-next / .app-bk-back / .app-bk-prog-step (shared classes)

   Options:
     prefix         (string)  required - id/attr prefix
     formId         (string)  override form id (default 'app-{prefix}-form')
...
(function () {
  ...
  window.chStepModal = function (opts) {
    ...
    var form = document.getElementById(
      opts.formId || "app-" + prefix + "-form",
    );
    var modal = document.getElementById("app-" + prefix + "-modal");
    var openBtn = document.getElementById("app-" + prefix + "-open");
    if (!form || !modal) return;

    var box = modal.querySelector(".app-bk-modal-box");
    var steps = Array.prototype.slice.call(
      form.querySelectorAll(".app-bk-step"),
    );
    var progSteps = Array.prototype.slice.call(
      modal.querySelectorAll(".app-bk-prog-step"),
    );
    var progFill = modal.querySelector(".app-bk-prog-fill");
    var msgEl = document.getElementById("app-" + prefix + "-msg");
    var submitBtn = document.getElementById("app-" + prefix + "-submit");
    var summary = document.getElementById("app-" + prefix + "-summary");
    ...
    function showMsg(text, type) {
      ...
      msgEl.className = "app-form-feedback " + type;
    ...
    function clearErrors() {
      form.querySelectorAll(".app-field-error").forEach(function (e) {
        e.remove();
      });
      form.querySelectorAll(".app-bk-field").forEach(function (e) {
    ...
    function openModal() {
      ...
      var prog = modal.querySelector(".app-bk-progress");
    ...
    function goTo(step) {
      ...
      if (progFill && total > 1)
        progFill.style.width = ((step - 1) / (total - 1)) * 100 + "%";
      if (
        step === total &&
        typeof opts.buildSummary === "function" &&
        summary
      ) {
    ...
    form.querySelectorAll(".app-bk-next").forEach(function (btn) {
      btn.addEventListener("click", function () {
        if (
          typeof opts.validateStep === "function" &&
          !opts.validateStep(ctx, current)
        )
          return;
        goTo(parseInt(btn.dataset.next, 10));
      });
    });
    form.querySelectorAll(".app-bk-back").forEach(function (btn) {
      btn.addEventListener("click", function () {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (
        emailEl &&
        (!emailEl.value.trim() || !EMAIL_RE.test(emailEl.value.trim()))
      ) {
      ...
      var data =
        typeof opts.collectData === "function"
          ? opts.collectData(ctx)
          : new FormData(form);
      data.append("action", "app_" + opts.action);
      ...
      function renderSuccess(message) {
        var lastStep = form.querySelector(
          '.app-bk-step[data-step="' + total + '"]',
        );
        if (lastStep) {
          lastStep.innerHTML =
            '<div class="app-bk-success">' +
            '<div class="app-bk-success-icon">' +
            (opts.successIcon || "🎉") +
            "</div>" +
            "<h3>" +
            (opts.successTitle || "Sent!") +
            "</h3>" +
            "<p>" +
            escHtml(
              message ||
            ...
            "</p>" +
            '<button type="button" class="btn-lime" ' +
            closeAttr +
            ' style="margin-top:1.2rem;">Close</button>' +
            "</div>";
...
(function ($) {
  ...
  function initContactForm() {
    ...
    var msg = document.getElementById("app-form-msg");
    var submit = document.getElementById("app-form-submit");
    if (!form) return;

    function showMsg(text, type) {
      if (!msg) return;
      msg.textContent = text;
      msg.className = "app-form-feedback " + type;
    ...
    function clearErrors() {
      form
        .querySelectorAll(".app-field-error:not(.app-consent-error)")
        .forEach(function (el) {
          el.remove();
        });
      ...
      var consentErr = form.querySelector(".app-consent-error");
      if (consentErr) consentErr.style.display = "none";
      var dg = form.querySelector(".app-disclaimer-group");
    ...
    function validate() {
      ...
      var consent = document.getElementById("app-consent");
      clearErrors();
      ...
      if (consent && !consent.checked) {
        ...
        consent.closest(".app-disclaimer-group").classList.add("has-error");
        ok = false;
      }
      return ok;
    }
    ...
    form.addEventListener("submit", function (e) {
      ...
      fetch(chTheme.ajaxUrl, { method: "POST", body: data })
        ...
        .then(function (res) {
          if (res.success) {
            var formContainer = form.closest(".app-contact-form");
            var message =
              res.data.message || "Thanks! We'll be in touch soon. 🌿";

            var successBox = document.createElement("div");
            successBox.style.cssText =
              "display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:500px;text-align:center;padding:4rem 2.5rem;";

            var emoji = document.createElement("div");
            emoji.textContent = "🌿";
            emoji.style.cssText =
              "font-size:5.5rem;margin-bottom:2rem;line-height:1;";

            var title = document.createElement("h3");
            title.textContent = "Message Sent!";
            title.style.cssText =
              "font-family:var(--app-font-display);font-size:2.2rem;font-weight:900;color:var(--client-color-1);margin-bottom:1rem;letter-spacing:-0.01em;";

            var msgText = document.createElement("p");
            msgText.textContent = message;
            msgText.style.cssText =
              "font-size:1.05rem;color:var(--client-color-15-muted);line-height:1.75;max-width:480px;margin:0;";
  ...
  function initNativeShare() {
    var shareBtn = document.getElementById("app-native-share");
    if (!shareBtn) return;
    ...
    shareBtn.addEventListener("click", function (e) {
      ...
      if (navigator.share) {
        navigator
          .share({ title: title, text: "Check this out!", url: url })
          .catch(function (err) {
  ...
  function initBookingWizard() {
    ...
    chStepModal({
      prefix: "bk",
      formId: "app-booking-form",
      action: "booking_submit",
      successTitle: "Order Request Sent!",
      successMessage:
        "Thanks! We'll be in touch very soon to confirm your order. 🌿",

      validateStep: function (ctx, step) {
        ctx.clearErrors();
        var canes = ctx.form.querySelectorAll(
          '[name="bk_cane[]"]:checked',
        ).length;
        var flav = ctx.form.querySelectorAll(
          '[name="bk_flavour[]"]:checked',
        ).length;
        if (step === 1 && canes === 0) {
          ctx.showMsg("Please choose at least one cane type. 🌾", "error");
          return false;
        }
        if (step === 2 && flav === 0) {
          ctx.showMsg("Please pick at least one flavour. 🍋", "error");
          return false;
        }
        if (step === 4) {
          var ok = true;
          ...
          if (!guests.value.trim() || parseInt(guests.value, 10) < 1) {
            ctx.fieldError(
              guests.closest(".app-bk-field"),
              "Please enter the number of guests (minimum 1).",
            );
            ok = false;
          }
          if (!location.value.trim()) {
            ctx.fieldError(
              location.closest(".app-bk-field"),
      ...
      buildSummary: function (ctx) {
        var canes = Array.prototype.map.call(
          ctx.form.querySelectorAll('[name="bk_cane[]"]:checked'),
          function (c) {
            return c.value;
          },
        );
        var flav = Array.prototype.map.call(
          ctx.form.querySelectorAll('[name="bk_flavour[]"]:checked'),
          function (c) {
            return c.value;
          },
        );
        ...
        var html =
          '<div class="app-bk-summary-title">Your Order Summary</div><div class="app-bk-summary-grid">';
        rows.forEach(function (r) {
          if (!r[1]) return;
          html +=
            '<div class="app-bk-summary-row"><span class="app-bk-summary-label">' +
            r[0] +
            '</span><span class="app-bk-summary-val">' +
            ctx.escHtml(r[1]) +
            "</span></div>";
        });
        return html + "</div>";
      ...
      collectData: function (ctx) {
        ...
        ctx.form
          .querySelectorAll('[name="bk_cane[]"]:checked')
          .forEach(function (c) {
            d.append("bk_cane[]", c.value);
          });
        ctx.form
          .querySelectorAll('[name="bk_flavour[]"]:checked')
          .forEach(function (c) {
            d.append("bk_flavour[]", c.value);
          });
        return d;
      },
    });
    ...
    chStepModal({
      ...
      validateStep: function (ctx, step) {
        ...
        if (step === 1) {
          ...
          if (!time.value.trim()) {
            ctx.fieldError(
              time.closest(".app-bk-field"),
              "Please select a timeline.",
            );
            ok = false;
          }
          if (!ok) {
            ctx.showMsg("Please fill in all required fields.", "error");
            return false;
          }
        }
        if (step === 2) {
          ...
          if (!inv.value.trim()) {
            ctx.fieldError(
              inv.closest(".app-bk-field"),
              "Please select an investment range.",
            );
            ctx.showMsg("Please select your investment range.", "error");
            return false;
          }
        }
        return true;
      ...
      buildSummary: function (ctx) {
        ...
        var html =
          '<div class="app-bk-summary-title">Your Enquiry Summary</div><div class="app-bk-summary-grid">';
        rows.forEach(function (r) {
          if (!r[1]) return;
          html +=
            '<div class="app-bk-summary-row"><span class="app-bk-summary-label">' +
  ...
  function initOrderWizard() {
    ...
    chStepModal({
      ...
      successTitle: "Order Request Sent!",
      successMessage:
        "Thanks! We'll review your order and contact you shortly. 🌿",
      // payload = all named fields (default new FormData(form))

      onInit: function (ctx) {
        // Quantity +/- buttons (otd-specific)
        ctx.form.querySelectorAll(".app-otd-qty-btn").forEach(function (btn) {
          btn.addEventListener("click", function () {
            var input = btn
              .closest(".app-otd-qty-wrap")
              .querySelector(".app-otd-qty-input");
            var v = parseInt(input.value, 10) || 1;
            input.value = btn.classList.contains("app-otd-qty-plus")
              ? v + 1
              : Math.max(1, v - 1);
          });
        });
      ...
      validateStep: function (ctx, step) {
        ctx.clearErrors();
        if (step === 1) {
          var items = ctx.form.querySelectorAll(
            '[name="otd_items[]"]:checked',
          ).length;
          var custom = ctx.form.querySelector('[name="otd_custom_item"]');
          if (items === 0 && (!custom || !custom.value.trim())) {
            ctx.showMsg("Please select at least one item. 🥤", "error");
            return false;
          }
        }
        if (step === 2) {
          ...
          if (!area.value.trim()) {
            ctx.fieldError(
              area.closest(".app-bk-field"),
              "Please enter your area or city.",
            );
            ok = false;
          }
          if (!ok) {
            ctx.showMsg("Please fill in your delivery details. 📦", "error");
            return false;
          }
        }
        return true;
      ...
      buildSummary: function (ctx) {
        var items = Array.prototype.map.call(
          ctx.form.querySelectorAll('[name="otd_items[]"]:checked'),
          function (c) {
            return c.value;
          },
        );
        var custom =
          (ctx.form.querySelector('[name="otd_custom_item"]') || {}).value ||
          "";
        ...
        if (custom) {
          var cqty =
            (ctx.form.querySelector('[name="otd_custom_qty"]') || {}).value ||
            "1";
...
(function () {
  ...
  if (typeof TOTAL_HISTORY_INFO === "undefined" || TOTAL_HISTORY_INFO <= 0)
    return;
  let current = 0;
  let busy = false;

  const pages = Array.from(document.querySelectorAll(".app-book-page"));
  const dots = Array.from(document.querySelectorAll(".app-hdot"));
  const ticks = Array.from(document.querySelectorAll(".app-hist-tick"));
  const pfill = document.getElementById("app-hist-pfill");
  const leaf = document.getElementById("app-turn-leaf");
  const prevBtn = document.getElementById("app-hist-prev");
  const nextBtn = document.getElementById("app-hist-next");
  ...
  function resetAll(activeIdx) {
    pages.forEach((p, i) => {
      ...
      p.querySelectorAll(".app-page-turn-trigger").forEach((b) =>
  ...
  function windToEnd(afterDone) {
    ...
    for (let i = current; i <= TOTAL_HISTORY_INFO - 2; i++) {
      pages[i].style.zIndex =
        TOTAL_HISTORY_INFO + (TOTAL_HISTORY_INFO - 2 - i) + 2;
    }
  ...
  function goTo(rawNext) {
    ...
    /* Wrap detection */
    const wrapsToStart =
      rawNext >= TOTAL_HISTORY_INFO; /* was on last, clicked Next  */
    ...
    outPage
      .querySelectorAll(".app-page-turn-trigger")
  ...
  pages.forEach((p, i) => {
    const trigger = p.querySelector(".app-page-turn-next");
  ...
  let touchStartX = 0;
  const book = document.getElementById("app-book");
  if (book) {
...
(function () {
  ...
  function initMobileNav() {
    var hamburger = document.getElementById("app-hamburger");
    var mobileNav = document.getElementById("app-mobile-nav");
    ...
    document.addEventListener("click", function (e) {
      if (
        !e.target.closest("#app-nav") &&
        !e.target.closest("#app-mobile-nav")
      ) {
        closeNav();
      }
    });
  ...
  function initNavPriority() {
    ...
    function adjust() {
      ...
      var inner = nav.querySelector(".app-nav__inner");
      if (!inner) return;
  ...
  function initSearchPanel() {
    var toggleBtn = document.getElementById("app-searnt-toggle");
    var panel = document.getElementById("app-searnt-panel");
    var closeBtn = document.getElementById("app-searnt-close");
    var input = document.getElementById("app-searnt-input");
      ...
      input.addEventListener("keydown", function (e) {
        if (e.key === "Enter" && input.value.trim()) {
          ...
          window.location.href =
            (typeof chTheme !== "undefined" ? chTheme.siteUrl : "/") +
            "?s=" +
            q;
        }
      });
  ...
  function initReviewCarousel() {
    var track = document.getElementById("app-reviews-track");
    var prev = document.getElementById("app-rev-prev");
    var next = document.getElementById("app-rev-next");
    if (!track) return;

    var cards = track.querySelectorAll(".app-review-card");
    var current = 0;
    var timer;

    function getDots() {
      return document.querySelectorAll("#app-nav-dots .app-dot");
    }
  ...
  function initJuiceShowcase() {
    var track = document.getElementById("app-showcase-track");
    var prev = document.getElementById("app-showcase-prev");
    var next = document.getElementById("app-showcase-next");
    if (!track) return;

    var cards = track.querySelectorAll(".app-showcase-card");
    var current = 0;
  ...
  function initCertsCarousel() {
    var track = document.getElementById("app-certs-track");
    var prev = document.getElementById("app-certs-prev");
    var next = document.getElementById("app-certs-next");
    if (!track) return;

    var cards = track.querySelectorAll(".app-cert-card");
    var current = 0;

    function getDots() {
      return document.querySelectorAll("#app-certs-dots .app-dot");
    }
  ...
  function initPkgCarousel() {
    var track = document.getElementById("app-pkg-track");
    var prev = document.getElementById("app-pkg-prev");
    var next = document.getElementById("app-pkg-next");
    if (!track) return;

    var cards = track.querySelectorAll(".app-package-card");
    var current = 0;

    function getDots() {
      return document.querySelectorAll("#app-pkg-dots .app-dot");
    }
    ...
    function show(idx) {
      ...
      cards.forEach(function (c, i) {
        const inRange = isDesktop
          ? i >= idx * visibleCount && i < (idx + 1) * visibleCount
          : i === idx;
      ...
      function () {
        ...
        raf = requestAnimationFrame(function () {
          ...
          if (
            idx !== current &&
            idx >= 0 &&
            idx < Math.ceil(cards.length / visibleCount)
          ) {
            current = idx;
            getDots().forEach(function (d, i) {
              d.classList.toggle("active", i === idx);
  ...
  function initEpcCarousel() {
    var track = document.getElementById("app-epc-track");
    var prev = document.getElementById("app-epc-prev");
    var next = document.getElementById("app-epc-next");
    if (!track) return;

    var cards = track.querySelectorAll(".app-epc");
    var current = 0;

    function getDots() {
      return document.querySelectorAll("#app-epc-dots .app-dot");
    }
  ...
  function makeFranchiseCarousel(trackId, prevId, nextId, dotsId, cardSel) {
    ...
    function getDots() {
      return document.querySelectorAll("#" + dotsId + " .app-dot");
    }
    ...
    function show(pageIdx) {
      ...
      if (window.innerWidth > 767 && cards[pageIdx * vc]) {
        cards[pageIdx * vc].scrollIntoView({
          behavior: "smooth",
          block: "nearest",
          inline: "start",
        });
    ...
    function rebuildDots() {
      ...
      for (var i = 0; i < total; i++) {
        ...
        btn.className = "app-dot" + (i === 0 ? " active" : "");
  ...
  function initGalleryStrips() {
    document.querySelectorAll(".app-gstrip").forEach(function (gstrip) {
      ...
      var cards = track.querySelectorAll(".app-gstrip__card");
      var current = 0;

      function getDots() {
        return document.querySelectorAll("#" + id + "-dots .app-dot");
      }
  ...
  function initRevEvCarousel() {
    var track = document.getElementById("app-rev-ev-track");
    var prev = document.getElementById("app-rev-ev-prev");
    var next = document.getElementById("app-rev-ev-next");
    if (!track) return;

    var cards = track.querySelectorAll(".app-rev-ev-card");
    var current = 0;

    function getDots() {
      return document.querySelectorAll("#app-rev-ev-dots .app-dot");
    }
  ...
  function initFaqAccordion() {
    var items = document.querySelectorAll(".app-faq-item");
    items.forEach(function (item) {
      var btn = item.querySelector(".app-faq-question");
      if (!btn) return;
      btn.addEventListener("click", function () {
        var isOpen = item.classList.contains("active");
        items.forEach(function (it) {
          it.classList.remove("active");
          var q = it.querySelector(".app-faq-question");
  ...
  function initFooterAccordion() {
    var toggles = document.querySelectorAll(".app-footer__acc-toggle");
    toggles.forEach(function (toggle) {
      var body = toggle.parentElement.querySelector(".app-footer__acc-body");
      if (!body) return;
  ...
  function initSmoothScroll() {
    document.querySelectorAll('a[href^="#"]').forEach(function (a) {
      a.addEventListener("click", function (e) {
        ...
        if (el) {
          e.preventDefault();
          var navH = document.getElementById("app-nav")
            ? document.getElementById("app-nav").offsetHeight
            : 72;
  ...
  function initStoryCards() {
    ...
    var tabs = Array.from(section.querySelectorAll(".app-sc-tab"));
    var panels = Array.from(section.querySelectorAll(".app-sc-panel"));
    var dots = Array.from(section.querySelectorAll(".app-sc-progress-dot"));
    var timer;
    ...
    function prevCard() {
      show((current() - 1 + tabs.length) % tabs.length);
      startTimer();
    }
    ...
    section.querySelectorAll(".app-sc-panel").forEach(function (panel) {
      var content = panel.querySelector(".app-sc-panel-content");
      var visual = panel.querySelector(".app-sc-panel-visual");
      var gallery = visual && visual.querySelector(".app-sc-gallery");
  ...
  function initStoryGalleries() {
    document.querySelectorAll(".app-sc-gallery").forEach(function (gallery) {
      var imgs = Array.prototype.slice.call(
        gallery.querySelectorAll(".app-sc-gallery-img"),
      );
      var dots = Array.prototype.slice.call(
        gallery.querySelectorAll(".app-sc-gallery-dot"),
      );
      ...
      function show(i) {
        ...
        imgs.forEach(function (im, n) {
          im.classList.toggle("active", n === idx);
        });
        dots.forEach(function (d, n) {
  ...
  function initScrollToTop() {
    var btn = document.getElementById("app-scroll-to-top");
    var fill = document.querySelector(".app-scroll-glass__fill");
    if (!btn) return;
  ...
  function initPostGalleries() {
    document.querySelectorAll(".app-post-gallery").forEach(function (gallery) {
      var id = gallery.getAttribute("id");
      var track = gallery.querySelector(".app-post-gallery__track");
      var slides = gallery.querySelectorAll(".app-post-gallery__slide");
      var dots = gallery.querySelectorAll(".app-post-gallery__dot");
      var arrows = gallery.querySelectorAll(".app-post-gallery__arrow");
      ...
      arrows.forEach(function (arrow) {
        var isNext = arrow.classList.contains("app-post-gallery__arrow--next");
  ...
  function initPrivacyModal() {
    var modal = document.getElementById("app-pp-modal");
    var overlay = document.getElementById("app-pp-overlay");
    if (!modal) return;
    ...
    function openModal(triggerEl) {
      ...
      modal.setAttribute("aria-hidden", "false");
      document.body.style.overflow = "hidden";
      var firstClose = modal.querySelector(".app-pp-close");
      if (firstClose)
        setTimeout(function () {
          firstClose.focus();
        }, 50);
    ...
    document.addEventListener("click", function (e) {
      var t = e.target.closest("#app-pp-trigger, .app-pp-trigger");
      if (t) {
        e.preventDefault();
        openModal(t);
      }
    ...
    modal.querySelectorAll(".app-pp-close").forEach(function (btn) {
  ...
  function initOneCardCarousels() {
    document.querySelectorAll("[data-oc]").forEach(function (root) {
      ...
      var dots = Array.prototype.slice.call(
        root.querySelectorAll("[data-oc-dots] [data-go]"),
      );
      var prev = root.querySelector("[data-oc-prev]");
      var next = root.querySelector("[data-oc-next]");
      var auto =
        parseInt(root.getAttribute("data-oc-autoplay") || "0", 10) || 0;
      var cur = 0,
        timer = null;
      ...
      function go(i, smooth) {
        ...
        track.scrollTo({
          left: items[cur].offsetLeft - items[0].offsetLeft,
      ...
      ["pointerdown", "touchstart", "mouseenter"].forEach(function (e) {
        track.addEventListener(e, stop, { passive: true });
      });
      ["pointerup", "touchend", "mouseleave"].forEach(function (e) {
        track.addEventListener(e, start, { passive: true });
      });
      window.addEventListener("resize", function () {
        setDots();
        start();
...
(function () {
  ...
  const dialog = document.getElementById("app-notice");
  const overlay = document.getElementById("app-notice-overlay");
   ...
   */
  function hashNotice() {
    const titleEl = dialog.querySelector(".app-notice-title");
    const msgEl = dialog.querySelector(".app-notice-message");
    const btnEl = dialog.querySelector(".app-notice-btn");
    const imgEl = dialog.querySelector(".app-notice-image img");
  ...
  function attachCloseHandlers(callback) {
    // Close button click
    const closeBtn = dialog.querySelector(".app-notice-close");
    if (closeBtn) {
