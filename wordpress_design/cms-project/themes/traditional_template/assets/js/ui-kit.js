...
(function () {
	...
	var TONES = CFG.tones || ['info', 'success', 'warning', 'error', 'note', 'question'];

	var DIALOG_PREFIX = 'nt-dialog-';
	...
	class NTStore {
		...
		add(id) {
			...
			} catch (e) { /* storage unavailable - the dismissal just won't persist */ }
		}
	}
	...
	class NTDialogView {
		...
		static build(data) {
			...
			dlg.className = ['nt-dialog', 'nt-dialog--' + d.tone, 'nt-dialog--' + d.size, d.cssClass]
				.filter(Boolean).join(' ');
			if (d.id) { dlg.id = d.id.indexOf(DIALOG_PREFIX) === 0 ? d.id : DIALOG_PREFIX + d.id; }
			if (!d.dismissible) { dlg.setAttribute('data-nt-dialog-locked', '1'); }
			if (d.autoOpen > 0) { dlg.setAttribute('data-nt-dialog-auto', String(d.autoOpen)); }
			if (d.once) { dlg.setAttribute('data-nt-dialog-once', d.once); }
			...
			if (d.dismissible) { paper.appendChild(NTDialogView.closeButton()); }
			paper.appendChild(NTDialogView.head(d));

			var body = NTDialogView.body(d);
			if (body) { paper.appendChild(body); }

			var actions = NTDialogView.actions(d);
			if (actions) { paper.appendChild(actions); }
		...
		static normalise(data) {
			...
			return {
				id: data.id || '',
				tone: tone,
				size: ['sm', 'md', 'lg', 'full'].indexOf(data.size) !== -1 ? data.size : (DEFAULTS.size || 'md'),
				icon: data.icon || tone,
				kicker: data.kicker || '',
				title: data.title || '',
				body: (body || []).filter(function (line) { return String(line).trim() !== ''; }),
				list: data.list || [],
				image: data.image || '',
				imageAlt: data.image_alt || '',
				form: (data.form && data.form.fields && data.form.fields.length) ? data.form : null,
		...
		static closeButton() {
			...
			btn.type = 'submit';
			btn.value = 'close';
			btn.className = 'nt-dialog__close';
			btn.setAttribute('aria-label', ARIA.close || t('close', 'Close'));
			if (ICONS.close) { btn.innerHTML = ICONS.close; }  // trusted: PHP-generated SVG
			return btn;
		}
		...
		static head(d) {
			...
			if (iconSvg) {
				...
				icon.className = 'nt-dialog__icon';
				...
				glyph.className = 'nt-dialog__icon nt-icon-emoji';
			...
			if (d.kicker) {
				...
				kicker.className = 'nt-dialog__kicker';
				kicker.textContent = d.kicker;
				head.appendChild(kicker);
			}

			var title = document.createElement('h2');
			title.className = 'nt-dialog__title';
			title.textContent = d.title;
			head.appendChild(title);

			var rule = document.createElement('span');
			rule.className = 'nt-dialog__rule';
		...
		static body(d) {
			if (!d.body.length && !d.list.length && !d.image && !d.form) { return null; }

			var wrap = document.createElement('div');
			wrap.className = 'nt-dialog__body';

			if (d.image) {
				var figure = document.createElement('figure');
				figure.className = 'nt-dialog__figure';
			...
			if (d.list.length) {
				...
				ul.className = 'nt-dialog__list';
				d.list.forEach(function (point) {
					var li = document.createElement('li');
					if (ICONS.check) {
						var tick = document.createElement('span');
						tick.className = 'nt-dialog__tick';
			...
			if (d.form) {
				...
				formWrap.className = 'nt-dialog__form';
		...
		static form(form) {
			...
			el.className = ['nt-form', form.class || ''].filter(Boolean).join(' ');
			if (form.id) { el.id = form.id; }
			if (form.action) { el.setAttribute('data-nt-ajax-form', form.action); }
			el.setAttribute('novalidate', '');

			(form.fields || []).forEach(function (field) {
				var group = document.createElement('div');
				group.className = 'nt-form-group nt-form-row';

				if (field.label) {
					var label = document.createElement('label');
					label.className = 'nt-form-label';
				...
				if (field.type === 'textarea') {
					...
					input.className = 'nt-form-textarea';
					input.rows = 5;
				} else if (field.type === 'select') {
					input = document.createElement('select');
					input.className = 'nt-form-select';
			...
			submit.type = 'submit';
			submit.className = 'nt-btn button nt-form-submit';
			submit.textContent = form.submit || t('submit', 'Send');
			el.appendChild(submit);

			var status = document.createElement('p');
			status.className = 'nt-form-status';
		...
		static actions(d) {
			...
			footer.className = 'nt-dialog__actions';

			d.actions.forEach(function (action) {
				if (!action || !action.label) { return; }
				var style = ['primary', 'ghost', 'danger'].indexOf(action.style) !== -1 ? action.style : 'primary';
				var cls = 'nt-dialog__btn nt-dialog__btn--' + style;
				var node;

				if (action.url) {
					node = document.createElement('a');
					node.href = action.url;
					if (action.new_tab) { node.target = '_blank'; node.rel = 'noopener noreferrer'; }
				} else {
					node = document.createElement('button');
					node.type = action.dialog_close === false ? 'button' : 'submit';
					node.value = action.value || 'ok';
					if (action.action) { node.setAttribute('data-nt-dialog-action', action.action); }
				}
				node.className = cls;
	...
	class NTDialog {
		constructor(element, options) {
			...
			this.opener = null;
			this.seen = new NTStore(DEFAULTS.dialogStore || 'nt_seen_dialogs');
			this.bind();
		}
		...
		bind() {
			...
			this.el.addEventListener('close', function () { self.onClosed(); });

			this.el.addEventListener('click', function (e) {
				// The <dialog> box fills the element, so a click that lands on
				// the element itself (not the paper) came from the backdrop.
				if (!self.locked && e.target === self.el) { self.close('backdrop'); }
			});

			this.el.addEventListener('cancel', function (e) {
				if (self.locked) { e.preventDefault(); }   // a locked dialog refuses Esc too
			});

			this.el.querySelectorAll('[data-nt-dialog-action]').forEach(function (btn) {
		...
		scheduleAutoOpen() {
			...
			if (onceKey && this.seen.has(onceKey)) { return; }

			var self = this;
			window.setTimeout(function () {
				// Never ambush someone already reading another dialog.
				if (document.querySelector('dialog.nt-dialog[open]')) { return; }
				self.open();
				if (onceKey) { self.seen.add(onceKey); }
			}, delay);
		}
		...
		open(opener) {
			...
			document.documentElement.classList.add('nt-dialog-open');
			this.focusFirst();
			return this;
		}
		...
		focusFirst() {
			var target = this.el.querySelector('[autofocus], input:not([type=hidden]), textarea, select, .nt-dialog__btn--primary')
				|| this.el.querySelector('.nt-dialog__close');
			if (target) { window.requestAnimationFrame(function () { target.focus(); }); }
		}

		onClosed() {
			this.el.classList.remove('is-open');
			if (!document.querySelector('dialog.nt-dialog[open]')) {
				document.documentElement.classList.remove('nt-dialog-open');
			}
		...
		runAction(name) {
			switch (name) {
				...
				default:
					document.dispatchEvent(new CustomEvent('nt:dialog-action', {
	...
	class NTDialogs {
		...
		scan() {
			...
			document.querySelectorAll('dialog.nt-dialog').forEach(function (el) {
				if (el.dataset.ntDialogInit) { return; }
				el.dataset.ntDialogInit = '1';
				var instance = new NTDialog(el);
				if (el.id) { self.instances.set(el.id, instance); }
			});

			document.querySelectorAll('[data-nt-dialog-open]').forEach(function (trigger) {
				if (trigger.dataset.ntDialogBound) { return; }
			...
			Object.keys(this.registry).forEach(function (domId) {
				if (self.registry[domId].auto_open > 0) { self.get(domId); }
			});
		...
		get(idOrEl) {
			...
			if (idOrEl.nodeType === 1) { return this.adopt(idOrEl); }

			var key = String(idOrEl);
			var domId = key.indexOf(DIALOG_PREFIX) === 0 ? key : DIALOG_PREFIX + key;

			if (this.instances.has(domId)) { return this.instances.get(domId); }

			var existing = document.getElementById(domId) || document.getElementById(key);
			if (existing) { return this.adopt(existing); }
			...
			if (window.NT && typeof window.NT.bindForms === 'function') { window.NT.bindForms(); }
			return instance;
		}

		/** Wrap an existing element in an NTDialog and remember it. */
		adopt(el) {
			if (el.id && this.instances.has(el.id)) { return this.instances.get(el.id); }
			var instance = new NTDialog(el);
			el.dataset.ntDialogInit = '1';
			if (el.id) { this.instances.set(el.id, instance); }
			return instance;
		}
		...
		show(data) {
			...
			return new Promise(function (resolve) {
				el.addEventListener('close', function () {
					...
					window.setTimeout(function () { el.remove(); }, reducedMotion() ? 0 : 220);
					resolve(value);
				});
				instance.open();
			});
		...
		alert(data) {
			data = (typeof data === 'string') ? { body: data } : Object.assign({}, data);
			if (!data.size) { data.size = 'sm'; }
			if (!data.actions) {
				data.actions = [{ label: data.okLabel || t('ok', 'OK'), style: 'primary', value: 'ok' }];
			}
			return this.show(data).then(function () { return undefined; });
		}

		/** Themed window.confirm. Resolves true only if the visitor agreed. */
		confirm(data) {
			data = (typeof data === 'string') ? { body: data } : Object.assign({}, data);
			if (!data.tone) { data.tone = 'question'; }
			if (!data.size) { data.size = 'sm'; }
			if (!data.actions) {
				data.actions = [
					{ label: data.cancelLabel || t('cancel', 'Cancel'), style: 'ghost', value: 'cancel' },
					{ label: data.okLabel || t('confirm', 'Confirm'), style: data.danger ? 'danger' : 'primary', value: 'ok' }
				];
			}
			return this.show(data).then(function (value) { return value === 'ok'; });
		}
	}
	...
	class NTConfirmTriggers {
		...
		scan() {
			...
			document.querySelectorAll('[data-nt-confirm]').forEach(function (node) {
				if (node.dataset.ntConfirmBound) { return; }
				...
				node.addEventListener(eventName, function (e) {
					...
					self.dialogs.confirm({
						...
						if (node.tagName === 'FORM') { node.submit(); } else { node.click(); }
					});
				});
			});
			return this;
	...
	class NTToaster {
		...
		container() {
			if (this.host && document.contains(this.host)) { return this.host; }
			this.host = document.createElement('div');
			this.host.className = 'nt-toasts';
		...
		show(message, tone, duration) {
			...
			node.className = 'nt-toast nt-toast--' + tone;

			if (ICONS[tone]) {
				var icon = document.createElement('span');
				icon.className = 'nt-toast__icon';
				icon.setAttribute('aria-hidden', 'true');
			...
			function dismiss() {
				...
				window.setTimeout(function () { node.remove(); }, reducedMotion() ? 0 : 320);
			}
	...
	class NTAlerts {
		...
		render(target, data, position) {
			...
			box.className = ['nt-alert', 'nt-alert--' + tone, data.compact ? 'nt-alert--compact' : '', data.class || '']
				.filter(Boolean).join(' ');
			box.setAttribute('role', (tone === 'error' || tone === 'warning') ? 'alert' : 'note');
			if (data.dismiss_id) { box.setAttribute('data-nt-alert-remember', data.dismiss_id); }

			if (ICONS[tone]) {
				var icon = document.createElement('span');
				icon.className = 'nt-alert__icon';
				icon.setAttribute('aria-hidden', 'true');
				icon.innerHTML = ICONS[tone];            // trusted: PHP-generated SVG
			...
			if (data.dismissible) {
				...
				close.type = 'button';
				close.className = 'nt-alert__close';
				close.setAttribute('data-nt-alert-close', '');
				close.setAttribute('aria-label', t('dismiss', 'Dismiss'));
				if (ICONS.close) { close.innerHTML = ICONS.close; }
				box.appendChild(close);
			}

			if (position === 'replace') { host.innerHTML = ''; host.appendChild(box); }
			else if (position === 'append') { host.appendChild(box); }
		...
		bindOne(box) {
			...
			if (remember && this.store.has(remember)) { box.remove(); return; }

			var btn = box.querySelector('[data-nt-alert-close]');
			if (!btn || btn.dataset.ntAlertBound) { return; }
			btn.dataset.ntAlertBound = '1';
			btn.addEventListener('click', function () {
				if (remember) { self.store.add(remember); }
		...
		scan() {
			var self = this;
			document.querySelectorAll('.nt-alert').forEach(function (box) { self.bindOne(box); });
			return this;
		}
	...
	class NTNotices {
		...
		render() {
			var self = this;
			var live = this.notices.filter(function (n) { return !self.store.has(n.id); });
			if (!live.length) { return null; }

			var host = document.createElement('div');
			host.className = 'nt-noticebar';
		...
		item(notice) {
			...
			var wrap = document.createElement('div');
			wrap.className = 'nt-noticebar__item nt-noticebar__item--' + toneOrDefault(notice.tone);
			wrap.setAttribute('data-nt-notice', notice.id);

			var inner = document.createElement('div');
			inner.className = 'nt-noticebar__inner';

			var iconSvg = ICONS[notice.icon] || ICONS[notice.tone];
			if (iconSvg) {
				var icon = document.createElement('span');
				icon.className = 'nt-noticebar__icon';
				icon.setAttribute('aria-hidden', 'true');
				icon.innerHTML = iconSvg;                     // trusted: PHP-generated SVG
			...
			if (notice.badge) {
				...
				badge.className = 'nt-noticebar__badge';
				badge.textContent = notice.badge;
				inner.appendChild(badge);
			}

			var text = document.createElement('p');
			text.className = 'nt-noticebar__text';
			if (notice.title) {
			...
			if (notice.button_label) {
				...
				if (notice.dialog) {
					...
					cta.type = 'button';
					cta.setAttribute('data-nt-dialog-open', DIALOG_PREFIX + notice.dialog);
				...
				if (cta) {
					cta.className = 'nt-noticebar__cta';
			...
			if (notice.dismissible) {
				var close = document.createElement('button');
				close.type = 'button';
				close.className = 'nt-noticebar__close';
				close.setAttribute('aria-label', t('dismiss', 'Dismiss'));
				if (ICONS.close) { close.innerHTML = ICONS.close; }
	...
	class NTTabs {
		constructor(scope) {
			this.scope = scope;
			this.tabs = Array.prototype.slice.call(scope.querySelectorAll('[data-nt-tab]'));
			this.panels = Array.prototype.slice.call(scope.querySelectorAll('[data-nt-tab-panel]'));
		}
		...
		init() {
			...
			this.tabs.forEach(function (tab, i) {
				tab.addEventListener('click', function () { self.select(tab.getAttribute('data-nt-tab')); });
			...
			var initial = this.scope.querySelector('[data-nt-tab].is-active') || this.tabs[0];
		...
		static scan() {
			document.querySelectorAll('[data-nt-tabs]').forEach(function (scope) {
				if (scope.dataset.ntTabsBound) { return; }
	...
	class NTPaperStory {
		constructor(root) {
			this.root = root;
			this.sheets = Array.prototype.slice.call(root.querySelectorAll('[data-nt-story-sheet]'));
			this.dots = Array.prototype.slice.call(root.querySelectorAll('[data-nt-story-dot]'));
		...
		init() {
			if (this.sheets.length < 2) { return this; }
			var self = this;
			this.root.classList.add('is-enhanced');

			if (this.prevBtn) { this.prevBtn.addEventListener('click', function () { self.stop(); self.go(self.index - 1); }); }
			if (this.nextBtn) { this.nextBtn.addEventListener('click', function () { self.stop(); self.go(self.index + 1); }); }

			this.dots.forEach(function (dot, i) {
				dot.addEventListener('click', function () { self.stop(); self.go(i); });
			});
			...
			this.root.addEventListener('keydown', function (e) {
				if (e.key === 'ArrowRight' || e.key === 'ArrowDown') { e.preventDefault(); self.stop(); self.go(self.index + 1); }
				if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') { e.preventDefault(); self.stop(); self.go(self.index - 1); }
			});
		...
		bindSwipe() {
			var self = this, startX = 0, startY = 0;
		...
		startAutoplay() {
			var ms = parseInt(this.root.getAttribute('data-nt-story-autoplay'), 10) || 0;
			if (!(ms > 0) || reducedMotion()) { return; }
			var self = this;
		...
		paint() {
			...
			this.sheets.forEach(function (sheet, i) {
				...
				sheet.style.setProperty('--nt-sheet-offset', String(Math.max(offset, -1)));
			...
			if (this.nextBtn) { this.nextBtn.disabled = this.index === this.sheets.length - 1; }
			if (this.counter) {
		...
		static scan() {
			document.querySelectorAll('[data-nt-story]').forEach(function (root) {
				if (root.dataset.ntStoryBound) { return; }
	...
	class NTLeafDrift {
		...
		init() {
			var ceiling = parseFloat(getComputedStyle(this.field).getPropertyValue('--nt-leaf-ceiling')) || 0.34;
			var rand = function (min, max) { return min + Math.random() * (max - min); };

			Array.prototype.forEach.call(this.field.querySelectorAll('.nt-leaf'), function (leaf, i) {
				leaf.style.setProperty('--nt-leaf-x', rand(-4, 104).toFixed(2) + '%');
				leaf.style.setProperty('--nt-leaf-delay', (-rand(0, 26)).toFixed(2) + 's');
				leaf.style.setProperty('--nt-leaf-duration', rand(16, 34).toFixed(2) + 's');
				leaf.style.setProperty('--nt-leaf-drift', rand(-90, 90).toFixed(0) + 'px');
				leaf.style.setProperty('--nt-leaf-spin', rand(-220, 220).toFixed(0) + 'deg');
				leaf.style.setProperty('--nt-leaf-scale', rand(0.55, 1.25).toFixed(2));
				leaf.style.setProperty('--nt-leaf-fade', rand(ceiling * 0.45, ceiling).toFixed(2));
				leaf.style.setProperty('--nt-leaf-sway', rand(3.5, 7.5).toFixed(2) + 's');
				leaf.style.setProperty('--nt-leaf-i', String(i));
			});
		...
		static scan() {
			...
			if (reducedMotion()) {
				fields.forEach(function (field) { field.remove(); });
				return;
			}
			fields.forEach(function (field) {
				if (field.dataset.ntLeavesBound) { return; }
	...
	class NTOverlayEscape {
		static scan() {
			...
			var selector = '.nt-bk-modal, .nt-modal, dialog.nt-dialog';

			document.querySelectorAll(selector).forEach(function (node) {
				if (node.dataset.ntEscaped) { return; }
	...
	class NTSideDock {
		...
		init() {
			...
			window.addEventListener('scroll', function () {
				...
				self.ticking = true;
				window.requestAnimationFrame(function () { self.onScroll(); });
			}, { passive: true });

			// Touching it always brings it back, however it got tucked.
			this.el.addEventListener('touchstart', function () { self.show(); }, { passive: true });
		...
		onScroll() {
			this.ticking = false;
			if (!this.mq.matches) { this.show(); return; }
			...
			if (goingDown && y > 220) { this.tuck(); }
			else if (goingUp) { this.show(); }

			// Stopping counts as "I might want a button now".
			var self = this;
			window.clearTimeout(this.idle);
			this.idle = window.setTimeout(function () { self.show(); }, 900);
		}

		tuck() { this.el.classList.add('is-tucked'); }
		show() { this.el.classList.remove('is-tucked'); }

		static scan() {
			document.querySelectorAll('.nt-floating-toolbar').forEach(function (el) {
				if (el.dataset.ntDockBound) { return; }
	...
	class NTMobileNav {
		...
		init() {
			...
			Array.prototype.forEach.call(
				this.nav.querySelectorAll('.nt-nav__has-sub'),
				function (item, i) {
					var link = item.querySelector('.nt-mobile-nav__link');
					var sub = item.querySelector('.nt-mobile-nav__submenu');
					if (!link || !sub) { return; }

					var id = 'nt-mnav-sub-' + i;
					sub.id = id;
					...
					if (!sub.querySelector('.nt-mobile-nav__subinner')) {
						var inner = document.createElement('div');
						inner.className = 'nt-mobile-nav__subinner';
					...
					toggle.type = 'button';
					toggle.className = 'nt-mobile-nav__toggle';
					...
					if (ICONS['chevron-down']) { toggle.innerHTML = ICONS['chevron-down']; }

					// A row holding the link and its toggle side by side.
					var row = document.createElement('div');
					row.className = 'nt-mobile-nav__row';
		...
		static scan() {
			document.querySelectorAll('.nt-mobile-nav').forEach(function (nav) {
				if (nav.dataset.ntMnavBound) { return; }
	...
	class NTShare {
		static scan() {
			if (!navigator.share) { return; }   // button stays hidden

			document.querySelectorAll('[data-nt-share]').forEach(function (scope) {
				if (scope.dataset.ntShareBound) { return; }
				...
				btn.addEventListener('click', function () {
					...
					}).catch(function () { /* the reader cancelled - not an error */ });
				});
			});
		}
	}
	...
	class NTCopy {
		...
		scan() {
			...
			document.querySelectorAll('[data-nt-copy]').forEach(function (btn) {
				if (btn.dataset.ntCopyBound) { return; }
				btn.dataset.ntCopyBound = '1';
				btn.addEventListener('click', function () {
					var value = btn.getAttribute('data-nt-copy');
					if (!value || !navigator.clipboard) { return; }
					navigator.clipboard.writeText(value)
						.then(function () { self.toaster.show(t('copied', ''), 'success'); })
						.catch(function () { self.toaster.show(t('error_generic', ''), 'error'); });
				});
			});
			return this;
		}
	...
	class NTUiKit {
		constructor() {
			var dismissStore = new NTStore(DEFAULTS.noticeStore || 'nt_dismissed_notices');
		...
		api() {
			...
			return {
				dialog: {
					open: function (id, opener) { return self.dialogs.open(id, opener); },
					close: function (id, value) { return self.dialogs.close(id, value); },
					show: function (data) { return self.dialogs.show(data); }
				},
				alert: function (data) { return self.dialogs.alert(data); },
				confirm: function (data) { return self.dialogs.confirm(data); },
				toast: function (message, tone, duration) { return self.toaster.show(message, tone, duration); },
				alerts: self.alerts,
				refresh: function () { return self.scan(); },
				// The classes themselves, for anything that needs to build on them.
				classes: {
					NTStore: NTStore, NTDialog: NTDialog, NTDialogView: NTDialogView,
					NTDialogs: NTDialogs, NTToaster: NTToaster, NTAlerts: NTAlerts,
					NTNotices: NTNotices, NTTabs: NTTabs, NTPaperStory: NTPaperStory,
