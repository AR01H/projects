 ...
 */
function app_require_theme_file( $rel ) {
 ...
 *
 *   app_component( 'parts/main_header' );
 *   app_component( 'cards/post_card', array( 'post_id' => 42 ) );
 *
 ...
 */
function app_component( $name, $context = array() ) {
 ...
 *
 *   app_option( 'general', 'phone', NT_BRAND_PHONE )
 *   app_option( 'social' )  -> the whole group array
 */
function app_option( $group, $key = null, $default = '' ) {
 ...
 *
 *   app_data( 'home' )          -> admin/data/home.json
 *   app_data( 'terms', array() )
 *
 ...
 * per request; alterable via the 'app_data_{name}' filter. Realpath-guarded.
 */
function app_data( $name, $default = array() ) {
	static $cache = array();
 ...
 *
 *   app_page_url( 'contact' )  ->  https://site.tld/contact/
 */
function app_page_url( $page_key ) {
