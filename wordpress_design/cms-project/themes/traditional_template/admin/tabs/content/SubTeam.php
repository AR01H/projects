...
?>
<div class="wrap">
<h2><?php echo $edit_id ? 'Edit Team Member' : 'Add New Team Member'; ?></h2>
<form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
	...
	<table class="form-table"><tbody>
		...
		<?php echo App_Admin_UI::text_field('role', 'Role / Title', $edit_item ? (string)($edit_item['role']??'') : '', true); ?>
		<?php echo App_Admin_UI::textarea_field('bio', 'Bio', $edit_item ? (string)($edit_item['bio']??'') : ''); ?>
		<?php echo App_Admin_UI::media_field('image_url', 'Photo', $edit_item ? (string)($edit_item['image_url']??'') : ''); ?>
		<?php echo App_Admin_UI::number_field('sort_order', 'Sort Order', $edit_item ? (string)($edit_item['sort_order']??'0') : '0'); ?>
	</tbody></table>
	<?php echo App_Admin_UI::submit_button('Save Team Member'); ?>
...
<table class="wp-list-table widefat fixed striped">
	...
	<tr>
		<td><?php echo esc_html($item['name']); ?></td><td><?php echo esc_html($item['role']); ?></td>
		<td>
			<a href="?page=<?php echo esc_attr($_GET['page']??''); ?>&subtab=<?php echo esc_attr($_GET['subtab']??''); ?>&edit=<?php echo $item['id']; ?>">✏️ Edit</a> |
			<a href="<?php echo wp_nonce_url(admin_url('admin-post.php?action=tt_delete_team&id='.$item['id']), 'tt_delete_team'); ?>" style="color:#c00;" onclick="return confirm('Delete this Team Member?');">🗑️ Delete</a>
		</td>
	</tr>
	<?php endforeach; ?>
