...
?>
<div class="wrap">
<h2><?php echo $edit_id ? 'Edit Value' : 'Add New Value'; ?></h2>
<form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
	<?php wp_nonce_field( 'tt_save_value' ); ?>
	<input type="hidden" name="action" value="tt_save_value">
	...
	<?php echo App_Admin_UI::submit_button('Save Value'); ?>
...
<table class="wp-list-table widefat fixed striped">
	...
	<tr>
		<td><?php echo esc_html($item['title']); ?></td><td><?php echo esc_html($item['description']); ?></td>
		<td>
			<a href="?page=<?php echo esc_attr($_GET['page']??''); ?>&subtab=<?php echo esc_attr($_GET['subtab']??''); ?>&edit=<?php echo $item['id']; ?>">✏️ Edit</a> |
			<a href="<?php echo wp_nonce_url(admin_url('admin-post.php?action=tt_delete_value&id='.$item['id']), 'tt_delete_value'); ?>" style="color:#c00;" onclick="return confirm('Delete this Value?');">🗑️ Delete</a>
		</td>
	</tr>
	<?php endforeach; ?>
