...
$table = $wpdb->prefix . 'tt_logo_strip';
...
?>
<div class="wrap">
<h2><?php echo $edit_id ? 'Edit Logo' : 'Add New Logo'; ?></h2>
<form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
	<?php wp_nonce_field( 'tt_save_logo_strip' ); ?>
	<input type="hidden" name="action" value="tt_save_logo_strip">
	...
	<table class="form-table"><tbody>
		<?php echo App_Admin_UI::text_field('name', 'Brand Name', $edit_item ? (string)($edit_item['name']??'') : '', false); ?>
		<?php echo App_Admin_UI::media_field('image_url', 'Logo Image', $edit_item ? (string)($edit_item['image_url']??'') : ''); ?>
		<?php echo App_Admin_UI::text_field('link_url', 'Link URL', $edit_item ? (string)($edit_item['link_url']??'') : '', false); ?>
		<?php echo App_Admin_UI::number_field('sort_order', 'Sort Order', $edit_item ? (string)($edit_item['sort_order']??'0') : '0'); ?>
	</tbody></table>
	<?php echo App_Admin_UI::submit_button('Save Logo'); ?>
...
<table class="wp-list-table widefat fixed striped">
	...
	<tr>
		...
		<td>
			...
			<a href="<?php echo wp_nonce_url(admin_url('admin-post.php?action=tt_delete_logo_strip&id='.$item['id']), 'tt_delete_logo_strip'); ?>" style="color:#c00;" onclick="return confirm('Delete this Logo?');">🗑️ Delete</a>
		</td>
	</tr>
	<?php endforeach; ?>
