...
$table = $wpdb->prefix . 'tt_delivery_products';
...
?>
<div class="wrap">
<h2><?php echo $edit_id ? 'Edit Delivery Product' : 'Add New Delivery Product'; ?></h2>
<form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
	<?php wp_nonce_field( 'tt_save_delivery_product' ); ?>
	<input type="hidden" name="action" value="tt_save_delivery_product">
	...
	<table class="form-table"><tbody>
		<?php echo App_Admin_UI::text_field('name', 'Product Name', $edit_item ? (string)($edit_item['name']??'') : '', true); ?>
		...
		<?php echo App_Admin_UI::text_field('price', 'Price', $edit_item ? (string)($edit_item['price']??'') : '', false); ?>
		<?php echo App_Admin_UI::number_field('sort_order', 'Sort Order', $edit_item ? (string)($edit_item['sort_order']??'0') : '0'); ?>
	</tbody></table>
	<?php echo App_Admin_UI::submit_button('Save Delivery Product'); ?>
...
<hr>
<h2>All Delivery Products</h2>
<table class="wp-list-table widefat fixed striped">
	<thead><tr><th>Name</th><th>Price</th><th>Description</th><th>Actions</th></tr></thead>
	<tbody>
	<?php foreach ($items as $item): ?>
	<tr>
		<td><?php echo esc_html($item['name']); ?></td><td><?php echo esc_html($item['price']); ?></td><td><?php echo esc_html($item['description']); ?></td>
		<td>
			<a href="?page=<?php echo esc_attr($_GET['page']??''); ?>&subtab=<?php echo esc_attr($_GET['subtab']??''); ?>&edit=<?php echo $item['id']; ?>">✏️ Edit</a> |
			<a href="<?php echo wp_nonce_url(admin_url('admin-post.php?action=tt_delete_delivery_product&id='.$item['id']), 'tt_delete_delivery_product'); ?>" style="color:#c00;" onclick="return confirm('Delete this Delivery Product?');">🗑️ Delete</a>
		</td>
	</tr>
	<?php endforeach; ?>
