 ...
 * Rows come from the app_submissions table (config/database.php); the form
		...
		<?php foreach ( $nt_rows as $nt_sub ) : ?>
			...
			<tr<?php echo $nt_is_new ? ' style="font-weight:600;"' : ''; ?>>
				...
				<td>
					...
					<?php if ( '' !== $nt_sub['phone'] ) : ?>
						<br><span class="app-muted"><?php echo esc_html( $nt_sub['phone'] ); ?></span>
					<?php endif; ?>
				</td>
				...
				<td><?php echo $nt_is_new ? '<span class="app-badge">' . esc_html__( 'new', NT_TEXT_DOMAIN ) . '</span>' : esc_html__( 'read', NT_TEXT_DOMAIN ); ?></td>
				<td>
					<?php
					$nt_row_form(
						'submission_status',
...
<?php if ( $nt_total_pages > 1 ) : ?>
	<p class="app-inbox-pages" style="margin-top:12px;">
