<?php
class TT_VisitorStats_Handler extends TT_Base_Handler {
    public static function handle_request() {
        ...
        if ( $action === 'create' ) {
            TT_Visitor_Stats_Model::insert( $data );
            self::redirect_success( 'Created successfully.' );
        } elseif ( $action === 'update' && $id > 0 ) {
            TT_Visitor_Stats_Model::update( $id, $data );
            self::redirect_success( 'Updated successfully.' );
        } elseif ( $action === 'delete' && $id > 0 ) {
            TT_Visitor_Stats_Model::delete( $id );
