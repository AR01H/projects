...
if (!function_exists('wpautop')) {
    function wpautop($pee) { return '<p>' . nl2br((string)$pee) . '</p>'; }
}

ob_start();
require __DIR__ . '/../pages/home/view.php';
$output = ob_get_clean();

echo "HOME PAGE RENDER SUCCESS: Generated " . strlen($output) . " bytes of HTML with zero errors!\n";

if (strpos($output, 'id="certifications"') !== false) {
    echo "CONFIRMED: Certifications section (FOOD SAFETY REGISTERED) is 100% rendered and printing in the Home Page HTML!\n";
} else {
    echo "ERROR: Certifications section is missing in Home Page HTML!\n";
}
