/* ==========================================================================
   ahStepModal — reusable multi-step modal wizard controller.
   ...
   Required DOM (all derived from prefix, e.g. 'consult'):
     #ah-{prefix}-form     the <form>            (or pass opts.formId)
     #ah-{prefix}-modal    the modal wrapper
     #ah-{prefix}-open     the button that opens it
     #ah-{prefix}-msg      feedback element
     #ah-{prefix}-submit   submit button
     #ah-{prefix}-summary  summary container (filled on the last step)
     [data-{prefix}-close] any close trigger
     .ah-bk-step / .ah-bk-next / .ah-bk-back / .ah-bk-prog-step (shared classes)

   Options:
     prefix         (string)  required — id/attr prefix
     formId         (string)  override form id (default 'ah-{prefix}-form')
     ...
     onInit         (fn ctx)              extra per-form wiring
...
(function () {
	...
	window.ahStepModal = function (opts) {
		opts = opts || {};
		var prefix = opts.prefix;
		if (!prefix || typeof ahTheme === 'undefined') return;

		var form    = document.getElementById(opts.formId || ('ah-' + prefix + '-form'));
		var modal   = document.getElementById('ah-' + prefix + '-modal');
		var openBtn = document.getElementById('ah-' + prefix + '-open');
		if (!form || !modal) return;

		var box       = modal.querySelector('.ah-bk-modal-box');
		var steps     = Array.prototype.slice.call(form.querySelectorAll('.ah-bk-step'));
		var progSteps = Array.prototype.slice.call(modal.querySelectorAll('.ah-bk-prog-step'));
		var progFill  = modal.querySelector('.ah-bk-prog-fill');
		var msgEl     = document.getElementById('ah-' + prefix + '-msg');
		var submitBtn = document.getElementById('ah-' + prefix + '-submit');
		var summary   = document.getElementById('ah-' + prefix + '-summary');
		var total     = steps.length;
		var current   = 1;
		var closeAttr = 'data-' + prefix + '-close';

		/* ── helpers ──────────────────────────────────────────────────────── */
		function showMsg(text, type) { if (!msgEl) return; msgEl.textContent = text; msgEl.className = 'ah-form-feedback ' + type; msgEl.style.display = 'block'; }
		function hideMsg() { if (msgEl) msgEl.style.display = 'none'; }
		function clearErrors() {
			form.querySelectorAll('.ah-field-error').forEach(function (e) { e.remove(); });
			form.querySelectorAll('.ah-bk-field').forEach(function (e) { e.classList.remove('invalid'); });
		}
		function fieldError(field, message) {
			if (!field) return;
			field.classList.add('invalid');
			var e = document.createElement('span');
			e.className = 'ah-field-error';
		...
		function openModal() {
			...
			var prog = modal.querySelector('.ah-bk-progress');
		...
		form.addEventListener('submit', function (e) {
			...
			data.append('nonce', ahTheme.nonce);

			fetch(ahTheme.ajaxUrl, { method: 'POST', body: data })
			...
			function renderSuccess(message) {
				var lastStep = form.querySelector('.ah-bk-step[data-step="' + total + '"]');
				if (lastStep) {
					lastStep.innerHTML =
						'<div class="ah-bk-success">' +
						'<div class="ah-bk-success-icon">' + (opts.successIcon || '✓') + '</div>' +
						'<h3>' + (opts.successTitle || 'Thank you!') + '</h3>' +
						'<p>' + escHtml(message || opts.successMessage || "We'll be in touch within one working day.") + '</p>' +
						'<button type="button" class="btn btn-primary" ' + closeAttr + ' style="margin-top:1.2rem;">Close</button>' +
						'</div>';
