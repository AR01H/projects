/* ==========================================================================
   carousel_video_scroll - slider behaviour (dots, arrows, swipe, video facade).
   Initialises every .ah-vs on the page; reads config from data- attributes.
...
(function () {
	...
	function initVideoScroll(root) {
		...
		var track  = root.querySelector('.ah-vs-track');
		var vp     = root.querySelector('.ah-vs-viewport');
		if (!track || !vp) return;

		var slides = Array.prototype.slice.call(track.querySelectorAll('.ah-vs-slide'));
		var dots   = Array.prototype.slice.call(root.querySelectorAll('.ah-vs-dot'));
		...
		function stopMedia() {
			...
			track.querySelectorAll('.ah-vs-media[data-yt-open="1"]').forEach(function (media) {
				var btn   = media.querySelector('.ah-vs-yt');
		...
		/* Arrows */
		var prev = root.querySelector('.ah-vs-arrow--prev');
		var next = root.querySelector('.ah-vs-arrow--next');
		...
		/* Play facade → load the player on click */
		function playFacade(btn) {
			var media = btn.closest('.ah-vs-media');
		...
		track.querySelectorAll('.ah-vs-yt').forEach(function (btn) {
	...
	function boot() {
		document.querySelectorAll('.ah-vs').forEach(initVideoScroll);
	}
