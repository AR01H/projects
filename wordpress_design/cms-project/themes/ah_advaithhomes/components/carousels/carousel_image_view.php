 ...
 *    'items'     => [
 *      [ 'type' => 'image', 'src' => '…/photo.jpg',  'label' => 'Property A' ],
 *      [ 'type' => 'video', 'src' => '…/clip.mp4', 'poster' => '…/thumb.jpg', 'desc' => 'Walkthrough' ],
 *    ],
 *  ] );
 *
 * ┌─────────────────────────────────────────────────────────────────┐
...
$uid         = esc_attr( $args['uid'] ?? 'ah-sc-' . wp_rand( 100, 999 ) );
...
foreach ( $items as $i => $item ) :
	...
	$is_vid = ( 'video' === $type );
	?>
	<div class="ah-sc-card">
		<div class="ah-sc-media">
			<?php if ( $is_vid ) : ?>
				<video class="ah-sc-vid" src="<?php echo $src; ?>"
					<?php if ( $poster ) : ?>poster="<?php echo $poster; ?>"<?php endif; ?>
					autoplay muted loop playsinline preload="metadata"></video>
				<span class="ah-sc-badge">▶ Video</span>
			<?php else : ?>
				<img class="ah-sc-img" src="<?php echo $src; ?>"
				     alt="<?php echo esc_attr( $label ); ?>" loading="lazy">
			<?php endif; ?>
		</div>
		<?php if ( $label || $desc ) : ?>
		<div class="ah-sc-caption">
...
<div class="ah-sc" id="<?php echo $uid; ?>" data-dir="<?php echo esc_attr( $direction ); ?>">
	<div class="ah-sc-viewport">
		<div class="ah-sc-track" id="<?php echo $uid; ?>-track">
			<?php
			/* Two identical passes → seamless infinite loop. aria-hidden on the clone. */
			echo $cards_html;                                  // phpcs:ignore -- pre-escaped above
			echo str_replace( 'class="ah-sc-card"', 'class="ah-sc-card" aria-hidden="true"', $cards_html ); // phpcs:ignore
			?>
		</div>
	</div>
</div><!-- .ah-sc -->
...
<style>
#<?php echo $uid; ?> .ah-sc-viewport { width: 100%; overflow: hidden; }

#<?php echo $uid; ?> .ah-sc-track {
	display: flex;
	flex-wrap: nowrap;
	width: max-content;
...
#<?php echo $uid; ?> .ah-sc-img,
#<?php echo $uid; ?> .ah-sc-vid {
...
@media (prefers-reduced-motion: reduce) {
	#<?php echo $uid; ?> .ah-sc-track { animation: none; }
}
</style>
