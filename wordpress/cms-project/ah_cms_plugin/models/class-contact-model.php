...
class AH_Contact_Model extends AH_Model_Base {
	...
	public function mark_email_sent( int $id ): bool {
		return AH_DB_Helper::update( $this->table(), array( 'email_sent' => 1, 'email_sent_at' => current_time( 'mysql' ) ), $id );
	}

	public function add_note( int $id, string $note ): bool {
		global $wpdb;
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT admin_notes FROM `{$this->table()}` WHERE id = %d", $id ) );
		if ( ! $row ) return false;
		$ts       = current_time( 'Y-m-d H:i' );
		$existing = $row->admin_notes ? rtrim( $row->admin_notes ) . "\n" : '';
		return AH_DB_Helper::update( $this->table(), array( 'admin_notes' => $existing . "[{$ts}] " . $note ), $id );
	}

	public function submit( array $data ): int|false {
		$allowed_types = array( 'general', 'complaint', 'sales', 'support', 'media', 'other' );
		$enquiry_type  = in_array( $data['enquiry_type'] ?? '', $allowed_types, true )
			? $data['enquiry_type']
			: 'general';
		...
		return AH_DB_Helper::insert( $this->table(), array(
			...
			'attachment_path' => sanitize_text_field( $data['attachment_path'] ?? '' ),
