...
add_action('admin_enqueue_scripts', function($hook) {
    if (strpos($hook, 'ch-content') === false && strpos($hook, 'ch-leads') === false && strpos($hook, 'ch-site-settings') === false) return;
    wp_enqueue_media();
...
add_action('wp_ajax_ch_save_item', function() {
    ...
    $tabs  = ch_tabs();
    if (!isset($tabs[$tab])) wp_send_json_error('Unknown tab');
...
add_action('wp_ajax_ch_delete_item', function() {
    ...
    $id   = intval($_POST['id'] ?? 0);
    $tabs = ch_tabs();
    if (!isset($tabs[$tab]) || !$id) wp_send_json_error('Invalid');
...
add_action('wp_ajax_ch_save_order', function() {
    ...
    $tabs  = ch_tabs();
    if (!isset($tabs[$tab])) wp_send_json_error('Invalid');
    $table = $wpdb->prefix . $tabs[$tab]['table'];
...
function ch_content_manager_page() {
    ...
    if (!isset($tabs[$active_tab])) $active_tab = 'reviews';
    ...
    <div class="wrap ch-wrap">
      ...
      <p class="ch-subtitle">Manage all website content from here. Each tab controls one section of the website. Changes go live instantly.</p>
          ...
          <table class="ch-items-table" id="ch-sortable-table">
            <thead>
              <tr>
                ...
                <th>#</th>
                <?php foreach ($fields as $f): if (in_array($f['type'], array('status','number')) && $f['name'] === 'sort_order') continue; ?>
            ...
            <tbody id="ch-sortable-body">
              <?php if (empty($rows)): ?>
              <tr><td colspan="20" class="ch-empty">No items yet. Click "Add New" to get started.</td></tr>
              <?php else: ?>
              <?php foreach ($rows as $row): ?>
              <tr class="ch-item-row <?php echo $row->status === 'inactive' ? 'ch-inactive' : ''; ?>" data-id="<?php echo $row->id; ?>">
                ...
                <td class="ch-cell-<?php echo esc_attr($f['name']); ?>">
                  <?php
                  if ($f['type'] === 'image') {
                      if ($val) echo '<img src="'.esc_url($val).'" class="ch-thumb" alt="">';
                      else echo '<span class="ch-no-img">No image</span>';
                  } elseif ($f['type'] === 'textarea') {
                ...
                <!-- STATUS -->
                <td>
                  <button class="ch-status-toggle ch-status-<?php echo $row->status; ?>"
                ...
                <td class="ch-actions-cell">
                  <button class="ch-edit-btn ch-btn-sm" data-id="<?php echo $row->id; ?>" data-tab="<?php echo $active_tab; ?>">✏️ Edit</button>
                  <button class="ch-delete-btn ch-btn-sm ch-btn-danger" data-id="<?php echo $row->id; ?>" data-tab="<?php echo $active_tab; ?>" data-name="<?php echo esc_attr($row->{array_column($fields,'name')[0]} ?? 'this item'); ?>">🗑️ Delete</button>
                </td>
              </tr>
              <?php endforeach; ?>
              <?php endif; ?>
            ...
            <div class="ch-form-fields">
              ...
              <div class="ch-form-group <?php echo $f['type'] === 'image' ? 'ch-full-width' : ''; ?>">
                ...
                if ($f['type'] === 'text' || $f['type'] === 'number') {
                    ...
                    foreach ($f['options'] as $val => $lbl) echo '<option value="'.esc_attr($val).'">'.esc_html($lbl).'</option>';
                    echo '</select>';
                } elseif ($f['type'] === 'image') {
