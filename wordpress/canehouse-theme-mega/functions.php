...
function canehouse_enqueue() {
    ...
    // Main JS
    wp_enqueue_script('ch-script',
        get_template_directory_uri() . '/assets/js/script.js',
        array(), '2.0', true
    );
    // Contact form AJAX JS
    wp_enqueue_script('ch-contact',
        get_template_directory_uri() . '/assets/js/contact-form.js',
        array('ch-script'), '2.0', true
    );
...
function ch_opt($key, $fallback = null) {
    $defaults = ch_opt_defaults();
    $stored   = get_option('ch_site_' . $key, null);
...
function ch_announcement_banner() {
    if (ch_opt('header_notice_on') !== '1') return;
